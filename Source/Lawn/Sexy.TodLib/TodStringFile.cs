...
internal static class TodStringFile
{
	...
	public static string TodStringTranslate(string theString)
	{
		...
		{
			if (!todStringTranslateCache.TryGetValue(theString, out var value))
			{
	...
	{
		return TodDrawStringWrapped(g, theText, theRect, theFont, theColor, theJustification, widowSupression: false, lineBreakChar);
	}
	...
	{
		return TodDrawStringWrappedHeight(g, theText, theRect, theFont, theColor, theJustification, widowSupression: false);
	}
	...
	public static int TodDrawStringWrappedHeight(Graphics g, string theText, TRect theRect, Font theFont, SexyColor theColor, DrawStringJustification theJustification, bool widowSupression)
	{
		...
		{
			int num = TodDrawStringWrappedHelper(g, theText2, theRect2, theFont, theColor, theJustification, drawString: false, widowSupression);
			theRect2.mY += (theRect.mHeight - num) / 2;
		}
		return TodDrawStringWrappedHelper(g, theText2, theRect2, theFont, theColor, theJustification, drawString: false, widowSupression);
	}
	...
	public static int TodDrawStringWrapped(Graphics g, string theText, TRect theRect, Font theFont, SexyColor theColor, DrawStringJustification theJustification, bool widowSupression, char lineBreakChar)
	{
		...
		return TodDrawStringWrappedHelper(g, theText2, theRect2, theFont, theColor, theJustification, drawString: true, widowSupression, lineBreakChar);
	}
		...
		do
		{
			...
			if (num3 >= 0)
			{
				if (!char.IsWhiteSpace(theValue[num3 + 1]))
				{
					...
					theValue = theValue.Insert(num3, "{" + num2 + "}");
					num2++;
				}
				else
				{
	...
	internal static void TodWriteStringSetFormat(string theFormat, ref TodStringListFormat theCurrentFormat)
	{
		...
		{
			TodStringListFormat todStringListFormat = gTodStringFormats[i];
			_ = todStringListFormat.mFormatName.Length;
	...
	internal static int TodWriteString(Graphics g, string theString, int theX, int theY, TodStringListFormat theCurrentFormat, int theWidth, DrawStringJustification theJustification, bool drawString, int theOffset, int theLength)
	{
		...
		if (drawString)
		{
			...
			case DrawStringJustification.DS_ALIGN_CENTER_VERTICAL_MIDDLE:
				theX += (theWidth - TodWriteString(g, theString, theX, theY, theCurrentFormat, theWidth, DrawStringJustification.DS_ALIGN_LEFT, drawString: false, theOffset, theLength)) / 2;
				break;
			case DrawStringJustification.DS_ALIGN_RIGHT:
			case DrawStringJustification.DS_ALIGN_RIGHT_VERTICAL_MIDDLE:
				theX += theWidth - TodWriteString(g, theString, theX, theY, theCurrentFormat, theWidth, DrawStringJustification.DS_ALIGN_LEFT, drawString: false, theOffset, theLength);
				break;
			}
		}
	...
	private static int strspn(string InputString, char[] Mask, int start)
	{
		...
		{
			if (!Enumerable.Contains<char>((IEnumerable<char>)Mask, InputString[i]))
			{
				flag = true;
				break;
			}
