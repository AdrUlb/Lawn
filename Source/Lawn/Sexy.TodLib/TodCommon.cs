...
internal static class TodCommon
{
	...
	{
		if ((theNumber & (uint)(1 << theBitIndex)) != 0)
		{
			return true;
		}
		return false;
	...
	public static void TodDrawImageCelScaledF(Graphics g, Image theImageStrip, float thePosX, float thePosY, int theCelCol, int theCelRow, float theScaleX, float theScaleY)
	{
		...
		SexyColor sexyColor = (g.mColorizeImages ? new SexyColor(g.mColor) : SexyColor.White);
		g.DrawImageTransformed(theImageStrip, ref theTransform, center: true, sexyColor, theSrcRect, clip: false);
	}
	...
	public static void TodDrawImageScaledF(Graphics g, Image theImage, float thePosX, float thePosY, float theScaleX, float theScaleY)
	{
		...
		theTransform.M22 = theScaleY;
		SexyColor sexyColor = (g.mColorizeImages ? new SexyColor(g.mColor) : SexyColor.White);
		g.DrawImageTransformed(theImage, ref theTransform, center: true, sexyColor, theSrcRect, clip: false);
	}
	...
	{
		TodDrawImageCenterScaledF(g, theImage, thePosX, thePosY, theScaleX, theScaleY, 0, doTexelOffset: false);
	}
	...
	public static void TodDrawImageCenterScaledF(Graphics g, Image theImage, float thePosX, float thePosY, float theScaleX, float theScaleY, int cel, bool doTexelOffset)
	{
		...
		theTransform.M22 = theScaleY;
		SexyColor sexyColor = (g.mColorizeImages ? new SexyColor(g.mColor) : SexyColor.White);
		g.DrawImageTransformed(theImage, ref theTransform, center: true, sexyColor, celRect, clip: false);
	}
	...
	public static void TodBltMatrix(Graphics g, Image theImage, ref Matrix theTransform, TRect theClipRect, SexyColor theColor, Graphics.DrawMode theDrawMode, TRect theSrcRect, bool clip)
	{
		...
		g.DrawImageTransformed(theImage, ref theTransform, center: true, theColor, theSrcRect, clip);
	}
	...
	{
		TodBltMatrix(g, theImage, ref theTransform, theClipRect, theColor, theDrawMode, theSrcRect, clip: false);
	}
	...
	public static void SexyMatrix3Translation(ref Matrix m, float x, float y)
	{
		...
		((Matrix)(ref m)).Translation = ((Matrix)(ref m)).Translation + new Vector3(x, y, 0f);
	}
	...
	public static void TodScaleTransformMatrix(ref Matrix m, float x, float y, float theScaleX, float theScaleY)
	{
		...
		Matrix val = Matrix.CreateTranslation(new Vector3(x, y, 0f));
		Matrix val2 = Matrix.CreateScale(new Vector3(theScaleX, theScaleY, 1f));
		m = val2 * val;
	}
	...
	public static void TodScaleRotateTransformMatrix(ref Matrix m, float x, float y, float rad, float theScaleX, float theScaleY)
	{
		...
		Matrix val = Matrix.CreateTranslation(new Vector3(x, y, 0f));
		Matrix val2 = Matrix.CreateRotationZ(0f - rad);
		Matrix val3 = Matrix.CreateScale(new Vector3(theScaleX, theScaleY, 1f));
		m = val3 * val2 * val;
	}
	...
	public static bool TodLoadResources(string theGroup)
	{
		...
		bool result = TodLoadResources(theGroup, doUnpackAtlasImages: false);
	...
	public static bool TodIsPointInPolygon(SexyVector2[] thePolygonPoint, int theNumberPolygonPoints, SexyVector2 theCheckPoint)
	{
		...
		{
			SexyVector2 sexyVector = thePolygonPoint[i];
			SexyVector2 sexyVector2 = default(SexyVector2);
			sexyVector2 = ((i != theNumberPolygonPoints - 1) ? thePolygonPoint[i + 1] : thePolygonPoint[0]);
			SexyVector2 sexyVector3 = (sexyVector2 - sexyVector).Perp();
			SexyVector2 v = theCheckPoint - sexyVector;
			if (sexyVector3.Dot(v) < 0f)
			{
				return false;
			}
		}
