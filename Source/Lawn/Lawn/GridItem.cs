...
internal class GridItem
{
	...
	public void DrawGridItem(Graphics g)
	{
		...
		mApp.ReanimationTryToGet(mGridItemReanimID)?.Draw(g);
		mApp.ParticleTryToGet(mGridItemParticleID)?.Draw(g, doScale: true);
	}
	...
	{
		mApp.ReanimationTryToGet(mGridItemReanimID)?.PlayReanim(GlobalMembersReanimIds.ReanimTrackId_anim_dissapear, ReanimLoopType.REANIM_PLAY_ONCE_AND_HOLD, 0, 12f);
		TodParticleSystem todParticleSystem = mApp.ParticleTryToGet(mGridItemParticleID);
		if (todParticleSystem != null)
		{
			todParticleSystem.ParticleSystemDie();
	...
	public void DrawScaryPot(Graphics g)
	{
		...
		if (mTransparentCounter > 0)
		{
			...
			if (mScaryPotType == ScaryPotType.SCARYPOT_SEED)
			{
				@new.mScaleX = 0.7f;
				@new.mScaleY = 0.7f;
				SeedPacket.DrawSmallSeedPacket(@new, (float)(num2 + Constants.GridItem_ScaryPot_SeedPacket_Offset.X) * Constants.S, (float)(num3 + Constants.GridItem_ScaryPot_SeedPacket_Offset.Y) * Constants.S, mSeedType, SeedType.SEED_NONE, 0f, 255, theDrawCost: false, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: false);
			}
