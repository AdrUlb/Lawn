...
internal class ContinueDialog : LawnDialog
{
	...
	public ContinueDialog(LawnApp theApp)
		: base(theApp, null, 37, isModal: true, "[CONTINUE_GAME_HEADER]", "", "[DIALOG_BUTTON_CANCEL]", 3)
	{
	...
	public override void Resize(int theX, int theY, int theWidth, int theHeight)
	{
		...
		int num2 = mLawnYesButton.mHeight;
		int theY2 = mLawnYesButton.mY - num2 - 2;
		int num3 = mLawnYesButton.mX - 20;
		int i;
		for (i = mLawnYesButton.mX + mLawnYesButton.mWidth - num + 24; num3 + num > i; i += 20)
		{
			num3 -= 20;
		}
		mContinueButton.Resize(num3, theY2, num, num2);
		mNewGameButton.Resize(i, mContinueButton.mY, num, num2);
		mLawnYesButton.Resize(theWidth / 2 - num / 2, mLawnYesButton.mY, num, num2);
	}
	...
	public override void ButtonDepress(int theId)
	{
		...
		case 1:
			...
			{
				LawnDialog lawnDialog = mApp.DoDialog(39, isModal: true, "[RESTART_LEVEL_HEADER]", "[RESTART_LEVEL]", "", 2);
			...
			{
				LawnDialog lawnDialog2 = mApp.DoDialog(39, isModal: true, "[NEW_GAME_HEADER]", "[NEW_GAME]", "", 2);
