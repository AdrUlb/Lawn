...
internal class SeedChooserScreen : Widget, StoreListener, AlmanacListener, LawnMessageBoxListener, SeedPacketsWidgetListener
{
	...
	public SeedChooserScreen()
	{
		...
		mSeedPacketsWidget = new SeedPacketsWidget(mApp, Has12Rows() ? 12 : 11, theIsImitaters: false, this);
		...
		ref SexyColor reference = ref mStartButton.mColors[1];
		reference = new SexyColor(255, 231, 26);
		ref SexyColor reference2 = ref mStartButton.mColors[0];
		reference2 = new SexyColor(255, 231, 26);
		...
		EnableStartButton(theEnabled: false);
		...
		ref SexyColor reference3 = ref mRandomButton.mColors[0];
		reference3 = new SexyColor(255, 240, 0);
		ref SexyColor reference4 = ref mRandomButton.mColors[1];
		reference4 = new SexyColor(200, 200, 255);
		...
		ref SexyColor reference5 = ref mViewLawnButton.mColors[0];
		reference5 = new SexyColor(42, 42, 90);
		ref SexyColor reference6 = ref mViewLawnButton.mColors[1];
		reference6 = new SexyColor(42, 42, 90);
		...
		ref SexyColor reference7 = ref mAlmanacButton.mColors[0];
		reference7 = new SexyColor(42, 42, 90);
		ref SexyColor reference8 = ref mAlmanacButton.mColors[1];
		reference8 = new SexyColor(42, 42, 90);
		...
		ref SexyColor reference9 = ref mStoreButton.mColors[0];
		reference9 = new SexyColor(42, 42, 90);
		ref SexyColor reference10 = ref mStoreButton.mColors[1];
		reference10 = new SexyColor(42, 42, 90);
	...
	public override void Draw(Graphics g)
	{
		...
		for (int i = 0; i < mNumPackets; i++)
		{
			...
			if (seedType != SeedType.SEED_NONE)
			{
				...
				SeedPacket.DrawSmallSeedPacket(g, x, y, seedType, chosenSeed.mImitaterType, 0f, 255, theDrawCost: true, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: true);
	...
	public override void DrawOverlay(Graphics g)
	{
		...
		for (int i = 0; i < 49; i++)
		{
			...
			SeedPacket.DrawSmallSeedPacket(g, chosenSeed.mX, chosenSeed.mY, chosenSeed.mSeedType, chosenSeed.mImitaterType, 0f, 255, theDrawCost: true, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: true);
		}
		if (flag)
		{
			g.EndHardwareClip();
	...
	public override void MouseDown(int x, int y, int theClickCount)
	{
		...
		for (int j = 0; j < 49; j++)
		{
			ChosenSeed theChosenSeed2 = mApp.mSeedChooserScreen.mChosenSeeds[j];
	...
	public void ClickedSeedInChooser(ref ChosenSeed theChosenSeed)
	{
		...
		if (mApp.mPlayerInfo.mNeedsGrayedPlantWarning && SeedNotRecommendedToPick(theChosenSeed.mSeedType) != 0)
		{
			...
			mApp.DoDialog(16, isModal: true, "[DIALOG_WARNING]", "[NOT_RECOMMENDED_FOR_LEVEL]", "[DIALOG_BUTTON_OK]", 3);
			return;
		}
		...
		if (mSeedsInBank == mBoard.mSeedBank.mNumPackets)
		{
			EnableStartButton(theEnabled: true);
		}
	}
	...
	public void EnableStartButton(bool theEnabled)
	{
		...
		if (theEnabled)
		{
			ref SexyColor reference = ref mStartButton.mColors[0];
			reference = new SexyColor(255, 231, 26);
		}
		else
		{
			ref SexyColor reference2 = ref mStartButton.mColors[0];
			reference2 = new SexyColor(64, 64, 64);
		}
	}
	...
	public int PickFromWeightedArrayUsingSpecialRandSeed(TodWeightedArray[] theArray, int theCount)
	{
		...
		Debug.ASSERT(num > 0);
		int num2 = (int)RandomNumbers.NextNumber((uint)num);
		int num3 = 0;
	...
	{
		_ = (StoreScreen)mApp.GetDialog(Dialogs.DIALOG_STORE);
		mApp.KillDialog(4);
	...
	public void SeedSelected(SeedType theSeedType)
	{
		...
		{
			mApp.DoDialog(16, isModal: true, string.Empty, "[NOT_ALLOWED_ON_THIS_LEVEL]", "[DIALOG_BUTTON_OK]", 3);
		}
