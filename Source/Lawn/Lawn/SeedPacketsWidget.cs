...
internal class SeedPacketsWidget : Widget
{
	...
	public void DrawPackets(Graphics g, bool theDrawCost, bool theDrawBackground)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (mApp.HasSeedType(seedType))
			{
				...
				if (chosenSeed.mSeedState != ChosenSeedState.SEED_IN_CHOOSER)
				{
					if (mImitaters)
					{
						SeedPacket.DrawSmallSeedPacket(g, theX, theY, SeedType.SEED_IMITATER, seedType, 0f, 255, theDrawCost, theUseCurrentCost: false, theDrawBackground, theDrawBackground);
					}
					else
					{
						SeedPacket.DrawSmallSeedPacket(g, theX, theY, seedType, SeedType.SEED_NONE, 0f, 55, theDrawCost, theUseCurrentCost: false, theDrawBackground, theDrawBackground);
					}
				}
			}
			else
		...
		for (int j = 0; j < 49; j++)
		{
			...
			if (chosenSeed2.mSeedState != ChosenSeedState.SEED_FLYING_TO_CHOOSER && chosenSeed2.mSeedState != 0 && chosenSeed2.mSeedState != ChosenSeedState.SEED_PACKET_HIDDEN && chosenSeed2.mSeedState == ChosenSeedState.SEED_IN_CHOOSER)
			{
				...
				if (mImitaters)
				{
					SeedPacket.DrawSmallSeedPacket(g, theX2, theY2, SeedType.SEED_IMITATER, chosenSeed2.mSeedType, 0f, (int)num2, theDrawCost, theUseCurrentCost: false, theDrawBackground, theDrawBackground);
				}
				else
				{
					SeedPacket.DrawSmallSeedPacket(g, theX2, theY2, chosenSeed2.mSeedType, chosenSeed2.mImitaterType, 0f, (int)num2, theDrawCost, theUseCurrentCost: false, theDrawBackground, theDrawBackground);
				}
			}
		}
	}
	...
	{
		g.HardwareClip();
		DrawPackets(g, theDrawCost: false, theDrawBackground: true);
		DrawPackets(g, theDrawCost: true, theDrawBackground: false);
		g.EndHardwareClip();
	}
}
