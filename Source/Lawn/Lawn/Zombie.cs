...
internal class Zombie : GameObject, IComparable
{
	...
	public void ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie theParentZombie, int theFromWave)
	{
		...
		default:
			if (mZombieType == ZombieType.ZOMBIE_BUNGEE)
			{
				...
				reanimation.SetTruncateDisappearingFrames(string.Empty, theTruncateDisappearingFrames: false);
			...
			if (mZombieType == ZombieType.ZOMBIE_DIGGER)
			{
				...
				reanimation2.SetTruncateDisappearingFrames(string.Empty, theTruncateDisappearingFrames: false);
				if (!IsOnBoard())
				{
			...
			case ZombieType.ZOMBIE_BALLOON:
			{
				...
				theAttachReanim.SetTruncateDisappearingFrames(string.Empty, theTruncateDisappearingFrames: false);
				if (IsOnBoard())
				{
	...
	public override bool LoadFromFile(Sexy.Buffer b)
	{
		...
		{
			SetRow(num);
			mPosX = mBoard.GridToPixelX(mTargetCol, num);
			mPosY = GetPosYBasedOnRow(num);
			mRenderOrder = Board.MakeRenderOrder(RenderLayer.RENDER_LAYER_GRAVE_STONE, num, 7);
		}
		return true;
	}
	...
	public override void LoadingComplete()
	{
		...
		if (mZombieType == ZombieType.ZOMBIE_BOSS)
		{
			...
			case 2:
				...
				ApplyBossSmokeParticles(theEnable: true);
				break;
			}
		}
		Update();
	...
	public void Update()
	{
		...
		if (mZombieFade > 0)
		{
			...
			if (mZombieFade <= 0)
			{
				DieNoLoot(giveAchievements: true);
			}
		}
		mX = (int)mPosX;
		mY = (int)mPosY;
	...
	public void Draw(Graphics g)
	{
		...
		{
			DrawIceTrap(g, ref theDrawPos, theFront: false);
		}
		...
		if (mIceTrapCounter > 0)
		{
			DrawIceTrap(g, ref theDrawPos, theFront: true);
		}
		...
		if (mAttachmentID != null)
		{
			...
			GlobalMembersAttachment.AttachmentDraw(mAttachmentID, @new, theParentHidden: false, doScale: true);
			@new.PrepareForReuse();
		}
		g.ClearClipRect();
	}
	...
	public void DrawBungeeCord(Graphics g, int theOffsetX, int theOffsetY)
	{
		...
		if (mZombiePhase == ZombiePhase.PHASE_ZOMBIE_BURNED)
		{
			int num3 = TodCommon.ClampInt((int)((float)(255 * mZombieFade) / 30f), 0, 255);
			g.SetColor(new Color(0, 0, 0, num3));
			g.SetColorizeImages(colorizeImages: true);
		}
		for (float num4 = thePosY - (float)num; num4 > (float)(-num); num4 -= (float)num)
		{
			float thePosX2 = (float)(theOffsetX + Constants.Zombie_Bungee_Offset.X) - 4f / mScaleZombie;
			float thePosY2 = num4 - mPosY - (float)Constants.Zombie_Bungee_Offset.Y;
			TodCommon.TodDrawImageScaledF(g, AtlasResources.IMAGE_BUNGEECORD, thePosX2, thePosY2, mScaleZombie, mScaleZombie);
		}
		if (mZombiePhase == ZombiePhase.PHASE_ZOMBIE_BURNED)
		{
			g.SetColor(mColor, premultiply: false);
	...
	public void UpdateZombieBungee()
	{
		...
		else if (mZombiePhase == ZombiePhase.PHASE_BUNGEE_RISING)
		{
			...
			if (mAltitude >= 600f)
			{
				DieNoLoot(giveAchievements: false);
			}
		}
	...
	public void UpdateZombieDolphinRider()
	{
		...
		else if (mZombiePhase == ZombiePhase.PHASE_DOLPHIN_RIDING)
		{
			if (mX <= 120)
			{
				...
				mZombiePhase = ZombiePhase.PHASE_DOLPHIN_WALKING;
				mAltitude = -40f;
				PoolSplash(theInToPoolSound: false);
				PlayZombieReanim(ref GlobalMembersReanimIds.ReanimTrackId_anim_walkdolphin, ReanimLoopType.REANIM_LOOP, 0, 0f);
				PickRandomSpeed();
				return;
			}
	...
	public void RiseFromGrave(int theCol, int theRow)
	{
		...
		if (mBoard.StageHasPool())
		{
			...
			mZombieHeight = ZombieHeight.HEIGHT_ZOMBIE_NORMAL;
			StartWalkAnim(0);
			ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_duckytube, theIgnoreClipRect: false);
			ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_whitewater, theIgnoreClipRect: false);
			ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_outerarm_hand, theIgnoreClipRect: false);
			ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_innerarm3, theIgnoreClipRect: false);
			Reanimation reanimation = mApp.ReanimationGet(mBodyReanimID);
	...
	public void UpdateZombieRiseFromGrave()
	{
		...
		if (mPhaseCounter <= 0)
		{
			...
			if (mInPool)
			{
				ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_duckytube, theIgnoreClipRect: true);
				ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_whitewater, theIgnoreClipRect: true);
				ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_outerarm_hand, theIgnoreClipRect: true);
				ReanimIgnoreClipRect(GlobalMembersReanimIds.ReanimTrackId_zombie_innerarm3, theIgnoreClipRect: true);
			}
		}
	}
	...
	public void CheckForPool()
	{
		...
		if (!mInPool && flag)
		{
			if (mBoard.mIceTrapCounter > 0)
			{
				...
				ApplyChill(theIsIceTrap: true);
				return;
			}
			mZombieHeight = ZombieHeight.HEIGHT_IN_TO_POOL;
			mInPool = true;
			PoolSplash(theInToPoolSound: true);
		}
	...
	public void UpdateZombieJackInTheBox()
	{
		...
		else
		{
			...
			if (mPhaseCounter <= 0)
			{
				...
				if (mMindControlled)
				{
					mBoard.KillAllZombiesInRadius(mRow, num, num2, Constants.JackInTheBoxZombieRadius, 1, theBurn: true, num3);
				}
				else
				{
					num3 |= 0x80;
	...
	public void OverrideParticleColor(TodParticleSystem aParticle)
	{
		if (aParticle != null)
		{
			if (mMindControlled)
			{
				...
				aParticle.OverrideExtraAdditiveDraw(null, theEnableExtraAdditiveDraw: true);
			}
			else if (mChilledCounter > 0 || mIceTrapCounter > 0)
			{
	...
	public bool HitIceTrap()
	{
		...
		if (mZombieType == ZombieType.ZOMBIE_BALLOON)
		{
			BalloonPropellerHatSpin(theSpinning: false);
		}
	...
	public void DrawReanim(Graphics g, ref ZombieDrawPosition theDrawPos, int theBaseRenderGroup)
	{
		...
		if (mZombieType == ZombieType.ZOMBIE_BOBSLED)
		{
			DrawBobsledReanim(g, ref theDrawPos, theBeforeZombie: true);
			reanimation.DrawRenderGroup(g, theBaseRenderGroup);
			DrawBobsledReanim(g, ref theDrawPos, theBeforeZombie: false);
		}
		else if (mZombieType == ZombieType.ZOMBIE_BUNGEE)
		{
	...
	public int TakeHelmDamage(int theDamage, uint theDamageFlags)
	{
		...
		if ((helmDamageIndex != helmDamageIndex2 && mBodyReanimID.mActive) || (justLoaded && mBodyReanimID.mActive))
		{
			...
			{
				mApp.ReanimationGet(mSpecialHeadReanimID)?.SetImageOverride(GlobalMembersReanimIds.ReanimTrackId_anim_face, AtlasResources.IMAGE_REANIM_WALLNUT_CRACKED1);
			}
			else if (mHelmType == HelmType.HELMTYPE_WALLNUT && helmDamageIndex2 == 2)
			{
				mApp.ReanimationGet(mSpecialHeadReanimID)?.SetImageOverride(GlobalMembersReanimIds.ReanimTrackId_anim_face, AtlasResources.IMAGE_REANIM_WALLNUT_CRACKED2);
			}
			else if (mHelmType == HelmType.HELMTYPE_TALLNUT && helmDamageIndex2 == 1)
			{
				mApp.ReanimationGet(mSpecialHeadReanimID)?.SetImageOverride(GlobalMembersReanimIds.ReanimTrackId_anim_idle, AtlasResources.IMAGE_REANIM_TALLNUT_CRACKED1);
			}
			else if (mHelmType == HelmType.HELMTYPE_TALLNUT && helmDamageIndex2 == 2)
			{
				mApp.ReanimationGet(mSpecialHeadReanimID)?.SetImageOverride(GlobalMembersReanimIds.ReanimTrackId_anim_idle, AtlasResources.IMAGE_REANIM_TALLNUT_CRACKED2);
			}
		}
		return result;
	}
	...
	public void UpdateReanim()
	{
		...
		if (reanimation2 != null)
		{
			...
			ref Matrix mMatrix = ref theOverlayMatrix.mMatrix;
			mMatrix.M11 *= reanimation.mOverlayMatrix.mMatrix.M11;
			ref Matrix mMatrix2 = ref theOverlayMatrix.mMatrix;
			mMatrix2.M21 *= reanimation.mOverlayMatrix.mMatrix.M11;
			ref Matrix mMatrix3 = ref theOverlayMatrix.mMatrix;
			mMatrix3.M12 *= reanimation.mOverlayMatrix.mMatrix.M22;
			ref Matrix mMatrix4 = ref theOverlayMatrix.mMatrix;
			mMatrix4.M22 *= reanimation.mOverlayMatrix.mMatrix.M22;
			ref Matrix mMatrix5 = ref theOverlayMatrix.mMatrix;
			mMatrix5.M41 *= reanimation.mOverlayMatrix.mMatrix.M11;
			ref Matrix mMatrix6 = ref theOverlayMatrix.mMatrix;
			mMatrix6.M42 *= reanimation.mOverlayMatrix.mMatrix.M22;
			ref Matrix mMatrix7 = ref theOverlayMatrix.mMatrix;
			mMatrix7.M41 += reanimation.mOverlayMatrix.mMatrix.M22;
			ref Matrix mMatrix8 = ref theOverlayMatrix.mMatrix;
			mMatrix8.M42 += reanimation.mOverlayMatrix.mMatrix.M42;
	...
	public void DropHead(uint theDamageFlags)
	{
		...
		Reanimation reanimation = mApp.ReanimationTryToGet(mBodyReanimID);
		if (mBoard.mMustacheMode && reanimation.TrackExists(GlobalMembersReanimIds.ReanimTrackId_zombie_mustache))
		{
			...
			Image imageOverride = reanimation.GetImageOverride(GlobalMembersReanimIds.ReanimTrackId_zombie_mustache);
		...
		{
			Image imageOverride2 = reanimation.GetImageOverride(GlobalMembersReanimIds.ReanimTrackId_anim_head1);
			int num = -1;
	...
	public void ApplyBurn()
	{
		...
		else
		{
			...
			if (mScaleZombie != 1f)
			{
				...
				ref Matrix mMatrix = ref reanimation4.mOverlayMatrix.mMatrix;
				mMatrix.M41 += (20f - mScaleZombie * 20f) * Constants.S;
				ref Matrix mMatrix2 = ref reanimation4.mOverlayMatrix.mMatrix;
				mMatrix2.M42 += (120f - mScaleZombie * 120f) * Constants.S;
				reanimation4.OverrideScale(mScaleZombie, mScaleZombie);
			}
			if (IsWalkingBackwards())
			{
				reanimation4.OverrideScale(0f - mScaleZombie, mScaleZombie);
				ref Matrix mMatrix3 = ref reanimation4.mOverlayMatrix.mMatrix;
				mMatrix3.M41 += 60f * mScaleZombie * Constants.S;
			}
			DieWithLoot();
		}
		if (mZombieType == ZombieType.ZOMBIE_BOBSLED)
	...
	public static void SetupReanimLayers(Reanimation aReanim, ZombieType theZombieType)
	{
		...
		SetupDoorArms(aReanim, theShow: false);
		...
		case ZombieType.ZOMBIE_PAIL:
			...
			break;
		case ZombieType.ZOMBIE_DOOR:
			SetupDoorArms(aReanim, theShow: true);
			break;
		case ZombieType.ZOMBIE_NEWSPAPER:
	...
	public void RemoveIceTrap()
	{
		...
		if (mZombieType == ZombieType.ZOMBIE_BALLOON)
		{
			BalloonPropellerHatSpin(theSpinning: true);
		}
		UpdateAnimSpeed();
		StartZombieSound();
	}
	...
	public void DrawBobsledReanim(Graphics g, ref ZombieDrawPosition theDrawPos, bool theBeforeZombie)
	{
		...
		Image theImage = num switch
		{
			0 => AtlasResources.IMAGE_ZOMBIE_BOBSLED1, 
			1 => AtlasResources.IMAGE_ZOMBIE_BOBSLED2, 
			2 => AtlasResources.IMAGE_ZOMBIE_BOBSLED3, 
			_ => AtlasResources.IMAGE_ZOMBIE_BOBSLED4, 
		};
		if (mZombiePhase == ZombiePhase.PHASE_ZOMBIE_BURNED)
		{
			g.SetColorizeImages(colorizeImages: true);
	...
	public void BobsledDie()
	{
		...
		if (!zombie.mDead)
		{
			zombie.DieNoLoot(giveAchievements: true);
		}
		for (int i = 0; i < 3; i++)
		{
			...
			if (!zombie2.mDead)
			{
				zombie2.DieNoLoot(giveAchievements: true);
			}
		}
	}
	...
	public void DrawDancerReanim(Graphics g, ref ZombieDrawPosition theDrawPos)
	{
		...
		if (mZombiePhase != ZombiePhase.PHASE_DANCER_DANCING_IN && mZombiePhase != ZombiePhase.PHASE_DANCER_SNAPPING_FINGERS && mZombiePhase != 0 && mZombiePhase != ZombiePhase.PHASE_ZOMBIE_DYING && mApp.mGameScene != GameScenes.SCENE_ZOMBIES_WON)
		{
			...
			{
			case 0:
				sexyColor = new SexyColor(250, 250, 160);
				break;
			case 1:
				sexyColor = new SexyColor(114, 234, 170);
				break;
			case 2:
				sexyColor = new SexyColor(216, 126, 202);
				break;
			case 3:
				sexyColor = new SexyColor(90, 110, 140);
				break;
			case 4:
				sexyColor = new SexyColor(240, 90, 130);
				break;
			}
			g.SetColorizeImages(colorizeImages: true);
	...
	private void SetupReanimForLostArm(uint theDamageFlags)
	{
		...
		if (reanimation != null)
		{
			...
			else if (mZombieType == ZombieType.ZOMBIE_FLAG)
			{
				...
				mApp.ReanimationTryToGet(mSpecialHeadReanimID)?.SetImageOverride(GlobalMembersReanimIds.ReanimTrackId_zombie_flag, AtlasResources.IMAGE_REANIM_ZOMBIE_FLAG3);
			}
			else if (mZombieType == ZombieType.ZOMBIE_DANCER)
			{
	...
	public void UpdateBoss()
	{
		...
		else if (mZombiePhase == ZombiePhase.PHASE_BOSS_HEAD_LEAVE)
		{
			...
			if (reanimation.mLoopCount > 0)
			{
				ApplyBossSmokeParticles(theEnable: false);
				BossPlayIdle();
			}
		}
		else
	...
	public void DrawBossBackArm(Graphics g, ref ZombieDrawPosition theDrawPos)
	{
		...
		Reanimation reanimation = mApp.ReanimationGet(mBodyReanimID);
		ref Matrix mMatrix = ref reanimation.mOverlayMatrix.mMatrix;
		mMatrix.M41 += num;
		ref Matrix mMatrix2 = ref reanimation.mOverlayMatrix.mMatrix;
		mMatrix2.M42 += num2;
		DrawReanim(g, ref theDrawPos, GameConstants.RENDER_GROUP_BOSS_BACK_ARM);
		ref Matrix mMatrix3 = ref reanimation.mOverlayMatrix.mMatrix;
		mMatrix3.M41 -= num;
		ref Matrix mMatrix4 = ref reanimation.mOverlayMatrix.mMatrix;
		mMatrix4.M42 -= num2;
	}
	...
	public static void PreloadZombieResources(ZombieType theZombieType)
	{
		...
		if (zombieDefinition.mReanimationType != ReanimationType.REANIM_NONE)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(zombieDefinition.mReanimationType, theIsPreloading: true);
		}
		if (theZombieType == ZombieType.ZOMBIE_DIGGER)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_DIGGER_DIRT, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED_DIGGER, theIsPreloading: true);
		}
		if (theZombieType == ZombieType.ZOMBIE_BOSS)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_BOSS_DRIVER, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_BOSS_FIREBALL, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_BOSS_ICEBALL, theIsPreloading: true);
			for (int i = 0; i < GameConstants.gBossZombieList.Length; i++)
			{
				...
				ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(zombieDefinition2.mReanimationType, theIsPreloading: true);
			}
		}
		if (theZombieType == ZombieType.ZOMBIE_DANCER)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_BACKUP_DANCER, theIsPreloading: true);
		}
		if (theZombieType == ZombieType.ZOMBIE_GARGANTUAR || theZombieType == ZombieType.ZOMBIE_REDEYE_GARGANTUAR)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_IMP, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED_IMP, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED_GARGANTUAR, theIsPreloading: true);
		}
		if (theZombieType == ZombieType.ZOMBIE_ZAMBONI)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_IMP, theIsPreloading: true);
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED_ZAMBONI, theIsPreloading: true);
		}
		if (theZombieType == ZombieType.ZOMBIE_CATAPULT)
		{
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED_CATAPULT, theIsPreloading: true);
		}
		ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_PUFF, theIsPreloading: true);
		ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_ZOMBIE_CHARRED, theIsPreloading: true);
		ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_LAWN_MOWERED_ZOMBIE, theIsPreloading: true);
	}
	...
	public void BossHeadSpitEffect()
	{
		...
		if (mIsFireBall)
		{
			...
			reanimation.GetCurrentTransform(theTrackIndex, out var aTransformCurrent, nullIfInvalidFrame: false);
		...
		else
		{
			...
			reanimation2.GetCurrentTransform(theTrackIndex2, out var aTransformCurrent2, nullIfInvalidFrame: false);
			...
			mApp.AddTodParticle(theX2, theY2, aRenderOrder, ParticleEffect.PARTICLE_ZOMBIE_BOSS_FIREBALL)?.OverrideImage(null, AtlasResources.IMAGE_ZOMBIE_BOSS_ICEBALL_PARTICLES);
	...
	public void UpdateZombiePeaHead()
	{
		...
		else
		{
			...
			if (reanimation != null)
			{
				int theTrackIndex = reanimation.FindTrackIndex(GlobalMembersReanimIds.ReanimTrackId_anim_head1);
				reanimation.GetCurrentTransform(theTrackIndex, out var aTransformCurrent, nullIfInvalidFrame: false);
	...
	public void UpdateZombieGatlingHead()
	{
		...
		else if ((mPhaseCounter >= 18 && mPhaseCounter < 21) || (mPhaseCounter >= 36 && mPhaseCounter < 39) || (mPhaseCounter >= 51 && mPhaseCounter < 54) || (mPhaseCounter >= 69 && mPhaseCounter < 72))
		{
			...
			if (reanimation2 != null)
			{
				...
				reanimation2.GetCurrentTransform(theTrackIndex, out var aTransformCurrent, nullIfInvalidFrame: false);
				float num = mPosX + aTransformCurrent.mTransX * Constants.IS - 9f;
