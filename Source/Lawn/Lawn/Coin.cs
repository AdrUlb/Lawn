...
internal class Coin : GameObject
{
	...
	public void CoinInitialize(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
	{
		...
		else if (mType == CoinType.COIN_PRESENT_PLANT)
		{
			...
			{
				int[] array = new int[8] { 0, 1, 2, 3, 7, 4, 5, 6 };
			...
			{
				int[] array2 = new int[8] { 8, 9, 10, 11, 12, 13, 14, 15 };
			...
			{
				int[] array3 = new int[8] { 16, 17, 18, 19, 20, 21, 22, 23 };
			...
			{
				int[] array4 = new int[8] { 24, 25, 26, 27, 28, 29, 30, 31 };
			...
			{
				int[] array5 = new int[6] { 32, 34, 35, 36, 37, 39 };
	...
	public void Draw(Graphics g)
	{
		...
		else
		{
			...
			if (mType == CoinType.COIN_FINAL_SEED_PACKET)
			{
				TodCommon.TodDrawImageCenterScaledF(g, AtlasResources.IMAGE_SEEDPACKETS, (int)(mPosX * Constants.S), (int)(mPosY * Constants.S), mScale, mScale, (int)LoadedSeedType, doTexelOffset: true);
				return;
			}
			...
			else
			{
				if (mType == CoinType.COIN_USABLE_SEED_PACKET)
				{
					...
					SeedPacket.DrawSmallSeedPacket(g, (int)(mPosX * Constants.S), (int)(mPosY * Constants.S), mUsableSeedType, SeedType.SEED_NONE, 0f, theGrayness, theDrawCost: false, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: false);
					g.SetColorizeImages(colorizeImages: false);
					return;
				}
				Debug.ASSERT(value: false);
	...
	public void Collect()
	{
		...
		if (mType == CoinType.COIN_AWARD_PRESENT || mType == CoinType.COIN_PRESENT_PLANT)
		{
			...
			if (mApp.mZenGarden.IsZenGardenFull(theIncludeDroppedPresents: false))
			{
	...
	public static int GetCoinValue(CoinType theType)
	{
		...
		{
			CoinType.COIN_SILVER => 1, 
			CoinType.COIN_GOLD => 5, 
			CoinType.COIN_DIAMOND => 100, 
			_ => 0, 
		};
	}
	...
	public SexyColor GetColor()
	{
		if ((IsSun() || IsMoney()) && mIsBeingCollected)
		{
			...
			return new SexyColor(255, 255, 255, (int)(255f * num), premultiply: false);
		}
		if (mFadeCount > 0)
		{
			int theAlpha = TodCommon.TodAnimateCurve(15, 0, mFadeCount, 255, 0, TodCurves.CURVE_LINEAR);
			return new SexyColor(255, 255, 255, theAlpha, premultiply: false);
		}
