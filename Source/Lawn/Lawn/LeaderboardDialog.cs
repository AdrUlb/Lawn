...
internal class LeaderboardDialog : LawnDialog
{
	...
	static LeaderboardDialog()
	{
		...
		loadingStrings = new string[3];
		loadingStrings[2] = text;
		loadingStrings[1] = text.Substring(0, text.Length - 1);
		loadingStrings[0] = text.Substring(0, text.Length - 2);
	}

	public LeaderboardDialog(LawnApp theApp, LeaderBoardType aType)
		: base(theApp, null, 2, isModal: true, "", "", "", 0)
	{
		mApp = theApp;
	...
	public override void Resize(int theX, int theY, int theWidth, int theHeight)
	{
		...
		mBackButton.Resize(mWidth / 2 - num / 2, theHeight - (int)Constants.InvertAndScale(50f), num, theHeight2);
	}
	...
	public override void Draw(Graphics g)
	{
		...
		if (num == -1)
		{
			TodCommon.TodDrawStringCenterBy(g, loadingStrings[loadingTimer], mWidth / 2, (int)Constants.InvertAndScale(150f), Resources.FONT_HOUSEOFTERROR16, SexyColor.White, DrawStringJustification.DS_ALIGN_CENTER, 1f, loadingStrings[0]);
		}
		else if (num == -2)
		{
			g.SetColor(Color.White);
			g.SetFont(Resources.FONT_HOUSEOFTERROR16);
			g.WriteWordWrapped(Constants.LeaderboardDialog_CannotConnect_Rect, TodStringFile.TodStringTranslate("[CANNOT_CONNECT]"), 0, 0, centerVertically: true);
		}
		else if (num > -1 && mListWidget == null)
		{
	...
	public override void Update()
	{
		...
		if (loadingStep % 10 == 0)
		{
			loadingTimer = (loadingTimer + 1) % loadingStrings.Length;
		}
	}
