...
internal class NewOptionsDialog : LawnDialog, SliderListener, CheckboxListener
{
	...
	public NewOptionsDialog(LawnApp theApp, bool theFromGameSelector)
		: base(theApp, null, 2, isModal: true, "[OPTIONS_DIALOG_TITLE]", "", "", 0)
	{
		...
		{
			mRestartButton.SetVisible(isVisible: false);
			mBackToMainButton.SetVisible(isVisible: false);
			mBackToGameButton.SetLabel("[DIALOG_BUTTON_OK]");
		}
		else
		{
			mTallBottom = mApp.mGameScene != GameScenes.SCENE_LEVEL_INTRO;
			mLinkCredits.SetVisible(isVisible: false);
		}
		...
		{
			mAlmanacButton.SetVisible(isVisible: false);
		}
		CalcSize(0, 0);
	}
	...
	public override void Resize(int theX, int theY, int theWidth, int theHeight)
	{
		...
		int theHeight2 = AtlasResources.IMAGE_BUTTON_LEFT.mHeight;
		...
		if (mFromGameSelector)
		{
			...
			mBackToGameButton.Resize(mWidth * 4 / 5 - num / 2, theHeight - (int)Constants.InvertAndScale(50f), num, theHeight2);
			mHelpButton.Resize(mWidth / 2 - num / 2, theHeight - (int)Constants.InvertAndScale(50f), num, theHeight2);
			mAboutButton.Resize(mWidth / 5 - num / 2, theHeight - (int)Constants.InvertAndScale(50f), num, theHeight2);
		...
		if (mApp.mGameScene == GameScenes.SCENE_LEVEL_INTRO)
		{
			...
			mBackToGameButton.Resize(mWidth / 4 - num2 / 2, theHeight - (int)Constants.InvertAndScale(50f), num2, theHeight2);
			mBackToMainButton.Resize(mWidth * 3 / 4 - num2 / 2, theHeight - (int)Constants.InvertAndScale(50f), num2, theHeight2);
			mVersionY = Constants.NewOptionsDialog_Version_Low_Y;
			return;
		}
		...
		{
			mBackToGameButton.Resize(theX3, theY2, num3, theHeight2);
			mRestartButton.Resize(theX4, theY2, num3, theHeight2);
			mBackToMainButton.Resize(theX3, theY3, num3, theHeight2);
			mAlmanacButton.Resize(theX4, theY3, num3, theHeight2);
		}
		else
		{
			mBackToGameButton.Resize(theX3, theY2, num3, theHeight2);
			mRestartButton.Resize(theX4, theY2, num3, theHeight2);
			mBackToMainButton.Resize(mWidth / 2 - num3 / 2 - (int)Constants.InvertAndScale(4f), theY3, num3, theHeight2);
		}
	...
	public override void CheckboxChecked(int theId, bool check)
	{
		...
		case 11:
		{
			...
			LawnDialog lawnDialog = mApp.DoDialog(53, isModal: true, empty, empty2, "", 3);
	...
	{
		int theId = mBackToGameButton.mId;
		ButtonPress(theId);
		ButtonDepress(theId);
		return true;
	}
	...
	public override void ButtonDepress(int theId)
	{
		...
		case 7:
		{
			...
			LawnDialog lawnDialog2 = mApp.DoDialog(52, isModal: true, empty3, empty4, "", 3);
		...
		case 2:
			if (mApp.mBoard != null)
			{
				...
				LawnDialog lawnDialog = mApp.DoDialog(23, isModal: true, empty, empty2, "", 1);
