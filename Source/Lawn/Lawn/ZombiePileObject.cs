...
internal struct ZombiePileObject
{
	...
	public ZombiePileObject(int aHeight, ZombiePileObjectType aType)
	{
		...
		mCounter = 0f;
		mY = -1;
		gemSpeedX = ((float)rand.NextDouble() - 0.5f) * 5f;
		if (Math.Abs(gemSpeedX) < 0.7f)
		{
			gemSpeedX = 0.7f;
		}
		gemSpeedY = ((float)rand.NextDouble() - 0.5f) * 0.5f;
		gemRotationSpeed = ((float)rand.NextDouble() - 0.5f) * 0.1f;
		switch (aType)
		{
		...
		{
			mCounter = rand.Next(Constants.BOARD_WIDTH);
		}
		...
		{
			mReanim = null;
			mOffsetX = rand.Next(Constants.BOARD_WIDTH);
		}
	}
}
