...
internal class CreditScreen : Widget, ButtonListener
{
	...
	internal static int DrawSection(Graphics g, string theLeftText, string theRightText, int theY)
	{
		...
		if (theLeftText.Length > 0 && theLeftText[0] == '^')
		{
			...
			if (!sectionSubstrings.TryGetValue(theLeftText, out var value))
			{
		...
		{
			if (!cachedHeightsLeft.TryGetValue(theLeftText, out var value2))
			{
	...
	public CreditScreen(LawnApp theApp)
	{
		...
		mMainMenuButton.SetVisible(isVisible: false);
		...
		mReplayButton.SetVisible(isVisible: false);
		mNumSections = 0;
		while (true)
		{
	...
	public void DrawCredits(Graphics g)
	{
		...
		g.HardwareClip();
		Resources.FONT_HOUSEOFTERROR16.EnableLayer(0, enabled: false);
		...
		Resources.FONT_HOUSEOFTERROR16.EnableLayer(0, enabled: true);
		g.EndHardwareClip();
	}
	...
	public void VideoFinished()
	{
		mVideoFinished = true;
		mVideoLoading = false;
		mReplayButton.SetVisible(isVisible: true);
		mMainMenuButton.SetVisible(isVisible: true);
