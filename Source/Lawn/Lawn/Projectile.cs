...
internal class Projectile : GameObject
{
	...
	internal static Projectile GetNewProjectile()
	{
		...
		{
			Projectile projectile = unusedObjects.Pop();
			projectile.Reset();
			return projectile;
		}
	...
	public void ProjectileInitialize(int theX, int theY, int theRenderOrder, int theRow, ProjectileType theProjectileType)
	{
		...
		else if (mProjectileType != ProjectileType.PROJECTILE_BUTTER)
		{
			...
			{
				mRotation = TodCommon.RandRangeFloat(0f, (float)Math.PI * 2f);
			...
			else if (mProjectileType == ProjectileType.PROJECTILE_STAR)
			{
				...
				{
					mRotationSpeed = 0f - mRotationSpeed;
				}
			}
	...
	public void DrawShadow(Graphics g)
	{
		...
		{
			float num6 = 200f / (TodCommon.ClampFloat(0f - mPosZ, 0f, 200f) + 200f);
			num3 *= num6;
		}
		TodCommon.TodDrawImageCelScaledF(g, AtlasResources.IMAGE_PEA_SHADOWS, num * Constants.S, (mShadowY - mPosY + num2) * Constants.S, theCelCol, 0, num3 * num4, num3);
	}
	...
	public void UpdateMotion()
	{
		...
		float posYBasedOnRow2 = mBoard.GetPosYBasedOnRow(mPosX, theRow);
	...
	public void CheckForCollision()
	{
		...
		else if (mPosX > (float)(Constants.WIDE_BOARD_WIDTH + 40) || mPosX + (float)mWidth < 0f)
		{
			Die();
		}
		...
		else if (mProjectileType == ProjectileType.PROJECTILE_STAR && (mPosY > 600f || mPosY < 0f))
		{
			Die();
		}
		else
		{
			if (((mProjectileType == ProjectileType.PROJECTILE_PEA || mProjectileType == ProjectileType.PROJECTILE_STAR) && mShadowY - mPosY > 90f) || mMotionType == ProjectileMotion.MOTION_FLOAT_OVER)
			{
				return;
			}
			if (mProjectileType == ProjectileType.PROJECTILE_ZOMBIE_PEA)
			{
				...
				if (zombie2 != null && (!zombie2.mOnHighGround || !CantHitHighGround()))
				{
					DoImpact(zombie2);
				}
			}
			...
			{
				Zombie zombie = FindCollisionTarget();
				if (zombie != null && (!zombie.mOnHighGround || !CantHitHighGround()))
				{
					DoImpact(zombie);
				}
			}
	...
	public void UpdateLobMotion()
	{
		...
		else if (mProjectileType == ProjectileType.PROJECTILE_COBBIG)
		{
			...
			mBoard.KillAllZombiesInRadius(mRow, (int)mPosX + 80, (int)mPosY + 40, 115, 1, theBurn: true, mDamageRangeFlags);
			...
			{
				mBoard.GrantAchievement(AchievementId.ACHIEVEMENT_POPCORN_PARTY, show: true);
			}
			DoImpact(null);
		}
		else
	...
	public void CheckForHighGround()
	{
		...
		else if (mProjectileType == ProjectileType.PROJECTILE_PUFF && num < 0f)
		{
			DoImpact(null);
		}
		else if (mProjectileType == ProjectileType.PROJECTILE_STAR && num < 23f)
		{
			DoImpact(null);
		}
		else if (CantHitHighGround())
		{
	...
	public bool CantHitHighGround()
	{
		...
		if (mProjectileType != 0 && mProjectileType != ProjectileType.PROJECTILE_SNOWPEA && mProjectileType != ProjectileType.PROJECTILE_STAR && mProjectileType != ProjectileType.PROJECTILE_PUFF && mProjectileType != ProjectileType.PROJECTILE_FIREBALL)
		{
			return false;
		}
		if (mOnHighGround)
	...
	public void UpdateNormalMotion()
	{
		...
		else if (mMotionType == ProjectileMotion.MOTION_HOMING)
		{
			...
			{
				TRect zombieRect = zombie.GetZombieRect();
				SexyVector2 sexyVector = new SexyVector2(zombie.ZombieTargetLeadX(0f), zombieRect.mY + zombieRect.mHeight / 2);
				SexyVector2 sexyVector2 = new SexyVector2(mPosX + (float)(mWidth / 2), mPosY + (float)(mHeight / 2));
				SexyVector2 sexyVector3 = (sexyVector - sexyVector2).Normalize();
				SexyVector2 sexyVector4 = new SexyVector2(mVelX, mVelY);
				sexyVector4.mVector += sexyVector3.mVector * (0.001f * (float)mProjectileAge);
				sexyVector4 = sexyVector4.Normalize();
				sexyVector4.mVector *= 2f;
				mVelX = sexyVector4.x;
				mVelY = sexyVector4.y;
				mRotation = (float)(0.0 - Math.Atan2(mVelY, mVelX));
			}
			mPosY += 3f * mVelY;
			mPosX += 3f * mVelX;
			mShadowY += 3f * mVelY;
		...
		else if (mMotionType == ProjectileMotion.MOTION_STAR)
		{
			...
			if (mVelY != 0f)
			{
				int num = mBoard.PixelToGridYKeepOnBoard((int)mPosX, (int)mPosY);
				mRow = num;
			}
		}
		...
		else if (mMotionType == ProjectileMotion.MOTION_FLOAT_OVER)
		{
			if (mVelZ < 0f)
			{
				...
				mPosY += 3f * mVelZ;
				mRotation = 0.3f + -0.70000005f * mVelZ * (float)Math.PI * 0.25f;
			}
			mPosX += 1.2f;
		}
	...
	{
		if (mProjectileType == ProjectileType.PROJECTILE_FIREBALL && theZombie != null && theZombie.IsFireResistant())
		{
			return false;
		}
		if (mProjectileType == ProjectileType.PROJECTILE_MELON || mProjectileType == ProjectileType.PROJECTILE_WINTERMELON || mProjectileType == ProjectileType.PROJECTILE_FIREBALL)
		{
			return true;
		}
		return false;
	...
	public bool PeaAboutToHitTorchwood()
	{
		...
		if (mProjectileType != 0 && mProjectileType != ProjectileType.PROJECTILE_SNOWPEA && mProjectileType != ProjectileType.PROJECTILE_ZOMBIE_PEA_MIND_CONTROL)
		{
			return false;
		}
		int count = mBoard.mPlants.Count;
