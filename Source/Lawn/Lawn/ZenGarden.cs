...
internal class ZenGarden : StoreListener
{
	...
	{
		DrawPottedPlant(g, x, y, thePottedPlant, 0.7f, theDrawPot: true);
	}
	...
	public void MouseDownWithTool(int x, int y, CursorType theCursorType)
	{
		...
		HitResult hitResult = mBoard.ToolHitTest(x, y, posScaled: true);
		Plant plant = null;
	...
	public void MovePlant(Plant thePlant, int theGridX, int theGridY)
	{
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZEN_GARDEN)
		{
			...
			_ = thePlant.mIsAsleep;
			thePlant.SetSleeping(theIsAsleep: false);
	...
	public void MouseDownWithMoneySign(Plant thePlant)
	{
		...
		LawnDialog lawnDialog = mApp.DoDialog(48, isModal: true, theDialogHeader, theDialogLines, "", 1);
	...
	public void GotoNextGarden()
	{
		...
		{
			mApp.KillBoard();
			mApp.PreNewGame(GameMode.GAMEMODE_TREE_OF_WISDOM, theLookForSavedGame: false);
			return;
		}
		...
		ZenGardenInitLevel(theJustSwitchingGardens: true);
	}
	...
	{
		thePlant.Die();
		mBoard.GetTopPlantAt(thePlant.mPlantCol, thePlant.mRow, PlantPriority.TOPPLANT_ONLY_UNDER_PLANT)?.Die();
	}
	...
	{
		HitResult hitResult = mApp.mBoard.ToolHitTest(x, y, posScaled: true);
		Plant plant = null;
		if (hitResult.mObjectType == GameObjectType.OBJECT_TYPE_PLANT)
		{
			plant = (Plant)hitResult.mObject;
		...
		if (plant != null || flag)
		{
			...
			if (flag)
			{
				newGridItem.mPosX = x;
				newGridItem.mPosY = y;
				Reanimation reanimation = mApp.AddReanimation(x + Constants.ZenGarden_GoldenWater_Pos.X, y + Constants.ZenGarden_GoldenWater_Pos.Y, 0, ReanimationType.REANIM_ZENGARDEN_WATERINGCAN, theDoScalePos: true);
	...
	{
		Plant topPlantAt = mBoard.GetTopPlantAt(thePlant.mPlantCol, thePlant.mRow, PlantPriority.TOPPLANT_ONLY_UNDER_PLANT);
		((topPlantAt == null) ? mApp.ParticleTryToGet(thePlant.mParticleID) : mApp.ParticleTryToGet(topPlantAt.mParticleID))?.ParticleSystemDie();
	}
	...
	public void UpdatePlantEffectState(Plant thePlant)
	{
		...
		{
			thePlant.SetSleeping(theIsAsleep: true);
		}
	}
	...
	public void StinkyUpdate(GridItem theStinky)
	{
		...
		SexyVector2 sexyVector = new SexyVector2(theStinky.mPosX, theStinky.mPosY);
		...
		while (mBoard.IterateCoins(ref theCoin))
		{
			...
			{
				sexyVector2.x = theCoin.mPosX;
				sexyVector2.y = theCoin.mPosY + 30f;
				float num = (sexyVector - sexyVector2).Magnitude();
				if (num < 20f)
				{
