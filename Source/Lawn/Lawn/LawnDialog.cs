...
internal class LawnDialog : Dialog
{
	...
	public virtual void SetButtonDelay(int theDelay)
	{
		...
		if (mLawnYesButton != null)
		{
			mLawnYesButton.SetDisabled(isDisabled: true);
		}
		if (mLawnNoButton != null)
		{
			mLawnNoButton.SetDisabled(isDisabled: true);
		}
	}
	...
	public override void Update()
	{
		...
		if (mUpdateCnt == mButtonDelay)
		{
			if (mLawnYesButton != null)
			{
				mLawnYesButton.SetDisabled(isDisabled: false);
			}
			if (mLawnNoButton != null)
			{
				mLawnNoButton.SetDisabled(isDisabled: false);
			}
		}
		MarkDirty();
	}
	...
	public override void Resize(int theX, int theY, int theWidth, int theHeight)
	{
		...
		if (mLawnYesButton != null && mLawnNoButton != null)
		{
			...
			mLawnYesButton.Resize(num9, num2, num8, theHeight2);
			mLawnNoButton.Resize(i, num2, num8, theHeight2);
		}
		else if (mLawnYesButton != null)
		{
			int num10 = num3 - AtlasResources.IMAGE_BUTTON_MIDDLE.mWidth + 1;
			num += (num3 - num10) / 2;
			mLawnYesButton.Resize(num, num2, num10, theHeight2);
		}
