...
internal static class GlobalMembersStoreScreen
{
	...
	internal static void DrawStoreItemIcon(Graphics g, int aPosX, int aPosY, StoreItem theItemType, int aQuantity)
	{
		...
		case StoreItem.STORE_ITEM_PACKET_UPGRADE:
		{
			...
			if (!seedNumberUpgradeCache.TryGetValue(aQuantity, out var value))
			{
		...
		case StoreItem.STORE_ITEM_PLANT_IMITATER:
			SeedPacket.DrawSmallSeedPacket(g, aPosX + Constants.StoreScreen_Imitater_Offset_X, aPosY + Constants.StoreScreen_Imitater_Offset_Y, SeedType.SEED_IMITATER, SeedType.SEED_NONE, 0f, 255, theDrawCost: false, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: false);
			break;
		...
		default:
			...
			else
			{
				...
				SeedPacket.DrawSmallSeedPacket(g, aPosX + -3, aPosY + Constants.StoreScreen_Default_Offset_Y, theSeedType, SeedType.SEED_NONE, 0f, 255, theDrawCost: false, theUseCurrentCost: false, theDrawBackground: true, theDrawCostBackground: false);
			}
			break;
		}
	}
