...
internal class Challenge : StoreListener
{
	...
	private int[] aDoubleChance = new int[6] { 0, 30, 10, 10, 15, 18 };

	private int[] aTripleChance = new int[6] { 0, 0, 0, 0, 10, 13 };

	private int[] aPailChance = new int[6] { 0, 0, 0, 10, 15, 15 };

	private int[] aConeChance = new int[6] { 0, 0, 30, 30, 30, 30 };

	static Challenge()
	{
		gArtChallengeWallnut = new SeedType[6, 9]
		{
			{
				SeedType.SEED_NONE,
				SeedType.SEED_NONE,
		...
		gArtChallengeSunFlower = new SeedType[6, 9]
		{
			{
				SeedType.SEED_NONE,
				SeedType.SEED_NONE,
		...
		gArtChallengeStarfruit = new SeedType[6, 9]
		{
			{
				SeedType.SEED_NONE,
				SeedType.SEED_NONE,
		...
		MAX_PICKS = (Constants.GRIDSIZEX + 1) * Constants.MAX_GRIDSIZEY;
		aGridPickItemArray = new GridItem[MAX_PICKS];
		aGridPicks = new TodWeightedArray[MAX_PICKS];
		aSeedPickArray = new TodWeightedArray[20];
		aPotArray = new TodWeightedArray[54];
		aGridArray = new TodWeightedGridArray[54];
		aPicks = new TodWeightedGridArray[Constants.GRIDSIZEX * Constants.MAX_GRIDSIZEY];
		for (int i = 0; i < aGridPickItemArray.Length; i++)
		{
			aGridPickItemArray[i] = GridItem.GetNewGridItem();
		}
		for (int j = 0; j < aGridPicks.Length; j++)
		{
			aGridPicks[j] = TodWeightedArray.GetNewTodWeightedArray();
		}
		for (int k = 0; k < aSeedPickArray.Length; k++)
		{
			aSeedPickArray[k] = TodWeightedArray.GetNewTodWeightedArray();
		}
		for (int l = 0; l < aPotArray.Length; l++)
		{
			aPotArray[l] = TodWeightedArray.GetNewTodWeightedArray();
		}
		for (int m = 0; m < aGridArray.Length; m++)
		{
			aGridArray[m] = TodWeightedGridArray.GetNewTodWeightedGridArray();
		}
		for (int n = 0; n < aPicks.Length; n++)
		{
			aPicks[n] = TodWeightedGridArray.GetNewTodWeightedGridArray();
		}
	}
	...
	private void Init()
	{
		...
		if (mApp.IsSlotMachineLevel())
		{
			...
			ReanimatorXnaHelpers.ReanimatorEnsureDefinitionLoaded(ReanimationType.REANIM_SLOT_MACHINE_HANDLE, theIsPreloading: true);
		...
		else if (mApp.mGameMode != GameMode.GAMEMODE_INTRO && mApp.mGameMode != 0)
		{
			mName = ChallengeScreen.gChallengeDefs[(int)(mApp.mGameMode - 1)].mChallengeName;
		}
	}
	...
	public void StartLevel()
	{
		...
		if (mApp.IsSurvivalMode() && mSurvivalStage == 0)
		{
			...
			empty = (mApp.IsSurvivalNormal(mApp.mGameMode) ? TodCommon.TodReplaceNumberString("[ADVICE_SURVIVE_FLAGS]", "{FLAGS}", 5) : ((!mApp.IsSurvivalHard(mApp.mGameMode)) ? "[ADVICE_SURVIVE_ENDLESS]" : TodCommon.TodReplaceNumberString("[ADVICE_SURVIVE_FLAGS]", "{FLAGS}", 10)));
			mBoard.DisplayAdvice(empty, MessageStyle.MESSAGE_STYLE_HINT_FAST, AdviceType.ADVICE_SURVIVE_FLAGS);
		}
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_LAST_STAND && mSurvivalStage == 0)
		{
			string theAdvice = TodCommon.TodReplaceNumberString("[ADVICE_SURVIVE_FLAGS]", "{FLAGS}", 5);
			mBoard.DisplayAdvice(theAdvice, MessageStyle.MESSAGE_STYLE_BIG_MIDDLE_FAST, AdviceType.ADVICE_SURVIVE_FLAGS);
		}
	...
	public int BeghouledVerticalMatchLength(int x, int y, BeghouledBoardState theBoardState)
	{
		...
		int i;
		for (i = 1; BeghouledGetPlantAt(x, y + i, theBoardState) == seedType; i++)
		{
		}
		return i;
	}
	...
	public int BeghouledHorizontalMatchLength(int x, int y, BeghouledBoardState theBoardState)
	{
		...
		int i;
		for (i = 1; BeghouledGetPlantAt(x + i, y, theBoardState) == seedType; i++)
		{
		}
		return i;
	}
	...
	public bool MouseMove(int x, int y)
	{
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_BEGHOULED && !mBoard.HasLevelAwardDropped())
		{
			...
			mBoard.MouseHitTest(x, y, out var theHitResult, posScaled: false);
	...
	public void BeghouledRemoveHorizontalMatch(int x, int y, BeghouledBoardState theBoardState)
	{
		...
		do
		{
			mBoard.GetTopPlantAt(x, y, PlantPriority.TOPPLANT_ANY)?.Die();
			x++;
		}
	...
	public void UpdateBeghouled()
	{
		...
		if (mChallengeStateCounter <= 0 && !mBoard.HasLevelAwardDropped())
		{
			...
			else if (mChallengeState == ChallengeState.STATECHALLENGE_BEGHOULED_NO_MATCHES)
			{
	...
	public bool UpdateBeghouledPlant(Plant thePlant)
	{
		...
		int val = ((mChallengeState != ChallengeState.STATECHALLENGE_BEGHOULED_MOVING) ? TodCommon.TodAnimateCurve(90, 30, mChallengeStateCounter, 1, 20, TodCurves.CURVE_EASE_IN) : 3);
		int num3 = 0;
		int num4 = 0;
		if (num > 0)
		{
			num3 = Math.Min(val, num);
			thePlant.mX += num3;
			flag = true;
		}
		else if (num < 0)
		...
		if (flag && (thePlant.mState == PlantState.STATE_MAGNETSHROOM_CHARGING || thePlant.mState == PlantState.STATE_MAGNETSHROOM_SUCKING))
		{
			...
			magnetItem.mPosX += num3;
			magnetItem.mPosY += num4;
		}
		return flag;
	}
	...
	public void BeghouledFallIntoSquare(int x, int y, BeghouledBoardState theBoardState)
	{
		...
		{
			Plant topPlantAt = mBoard.GetTopPlantAt(x, num, PlantPriority.TOPPLANT_ANY);
			if (topPlantAt != null)
			{
				topPlantAt.mRow = y;
				topPlantAt.mRenderOrder = topPlantAt.CalcRenderOrder();
				theBoardState.mSeedType[x, y] = topPlantAt.mSeedType;
				theBoardState.mSeedType[x, num] = SeedType.SEED_NONE;
	...
	public void BeghouledMakePlantsFall(BeghouledBoardState theBoardState)
	{
		for (int num = 4; num >= 0; num--)
		{
			for (int i = 0; i < 8; i++)
			{
				SeedType seedType = BeghouledGetPlantAt(i, num, theBoardState);
				if (seedType == SeedType.SEED_NONE)
				{
					BeghouledFallIntoSquare(i, num, theBoardState);
				}
			}
		}
	}
	...
	{
		if (mChallengeState == ChallengeState.STATECHALLENGE_NORMAL && !mBoard.HasLevelAwardDropped())
		{
			BeghouledBoardState newBeghouledBoardState = BeghouledBoardState.GetNewBeghouledBoardState();
			LoadBeghouledBoardState(newBeghouledBoardState);
	...
	public void InitZombieWavesSurvival()
	{
		...
		int num = Math.Min(mSurvivalStage + 1, 9);
		while (num > 0)
		{
	...
	public void InitZombieWaves()
	{
		if (mApp.IsSurvivalMode())
		{
			...
			{
				ZombieType[] array = new ZombieType[2]
				{
			...
			{
				ZombieType[] array2 = new ZombieType[3]
				{
					ZombieType.ZOMBIE_NORMAL,
					ZombieType.ZOMBIE_TRAFFIC_CONE,
		...
		{
			ZombieType[] array3 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
		...
		{
			ZombieType[] array4 = new ZombieType[1] { ZombieType.ZOMBIE_POGO };
			InitZombieWavesFromList(array4, array4.Length);
		}
		else if (mApp.IsBungeeBlitzLevel())
		{
			ZombieType[] array5 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_PAIL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
		...
		{
			ZombieType[] array6 = new ZombieType[6]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_PAIL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
		...
		{
			ZombieType[] array7 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_PAIL,
				ZombieType.ZOMBIE_FOOTBALL,
		...
		{
			ZombieType[] array8 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_FOOTBALL,
		...
		{
			ZombieType[] array9 = new ZombieType[6]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array10 = new ZombieType[8]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array11 = new ZombieType[5]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array12 = new ZombieType[3]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL
		...
		{
			ZombieType[] array13 = new ZombieType[2]
			{
		...
		{
			ZombieType[] array14 = new ZombieType[1] { ZombieType.ZOMBIE_BALLOON };
		...
		{
			ZombieType[] array15 = new ZombieType[6]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array16 = new ZombieType[10]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array17 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array18 = new ZombieType[6]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array19 = new ZombieType[2]
			{
		...
		{
			ZombieType[] array20 = new ZombieType[6]
			{
				ZombieType.ZOMBIE_PEA_HEAD,
				ZombieType.ZOMBIE_WALLNUT_HEAD,
		...
		{
			ZombieType[] array21 = new ZombieType[2]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE
			};
		...
		{
			ZombieType[] array22 = new ZombieType[5]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array23 = new ZombieType[7]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL,
		...
		{
			ZombieType[] array24 = new ZombieType[4]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_DOLPHIN_RIDER,
		...
		{
			ZombieType[] array25 = new ZombieType[3]
			{
				ZombieType.ZOMBIE_NORMAL,
				ZombieType.ZOMBIE_TRAFFIC_CONE,
				ZombieType.ZOMBIE_PAIL
	...
	public void UpdateSlotMachine()
	{
		...
		else
		{
			...
			mChallengeState = ChallengeState.STATECHALLENGE_NORMAL;
			SeedType mPacketType = mBoard.mSeedBank.mSeedPackets[0].mPacketType;
			SeedType mPacketType2 = mBoard.mSeedBank.mSeedPackets[1].mPacketType;
			SeedType mPacketType3 = mBoard.mSeedBank.mSeedPackets[2].mPacketType;
			if (mPacketType == mPacketType2 && mPacketType2 == mPacketType3)
			{
				...
				case SeedType.SEED_SLOT_MACHINE_DIAMOND:
				{
					...
					for (int i = 0; i < num2; i++)
					{
						...
						mBoard.AddCoin(theX, 85, CoinType.COIN_DIAMOND, CoinMotion.COIN_MOTION_COIN);
					}
					return;
				}
				case SeedType.SEED_SLOT_MACHINE_SUN:
				{
					...
					for (int j = 0; j < num3; j++)
					{
						...
						mBoard.AddCoin(theX2, 85, CoinType.COIN_SUN, CoinMotion.COIN_MOTION_COIN);
					}
					return;
				}
				}
				...
				for (int k = 0; k < num4; k++)
				{
					...
					Coin coin = mBoard.AddCoin(theX3, 85, CoinType.COIN_USABLE_SEED_PACKET, CoinMotion.COIN_MOTION_COIN);
					coin.mUsableSeedType = mPacketType;
				}
			}
			else
			{
				...
				mApp.PlayFoley(FoleyType.FOLEY_ART_CHALLENGE);
				SeedType seedType = ((mPacketType != mPacketType2 && mPacketType != mPacketType3) ? mPacketType2 : mPacketType);
				switch (seedType)
				{
				case SeedType.SEED_SLOT_MACHINE_DIAMOND:
				...
				case SeedType.SEED_SLOT_MACHINE_SUN:
				{
					...
					for (int l = 0; l < num5; l++)
					{
						...
						mBoard.AddCoin(theX4, 85, CoinType.COIN_SUN, CoinMotion.COIN_MOTION_COIN);
					}
					break;
				}
				default:
				{
					...
					Coin coin2 = mBoard.AddCoin(360, 85, CoinType.COIN_USABLE_SEED_PACKET, CoinMotion.COIN_MOTION_COIN);
					coin2.mUsableSeedType = seedType;
					break;
				}
				}
			}
	...
	public void DrawSlotMachine(Graphics g)
	{
		...
		if (mApp.mGameScene != GameScenes.SCENE_ZOMBIES_WON)
		{
			...
			if (mSlotMachineRollCount < 3 && mBoard.mCursorObject.mCursorType == CursorType.CURSOR_TYPE_NORMAL && mChallengeState != ChallengeState.STATECHALLENGE_SLOT_MACHINE_ROLLING && !mBoard.HasLevelAwardDropped())
			{
				...
				SexyColor sexyColor = new SexyColor(b, b, b, 255);
				@new.SetColorizeImages(colorizeImages: true);
				@new.SetDrawMode(Graphics.DrawMode.DRAWMODE_ADDITIVE);
				@new.SetColor(sexyColor);
				@new.DrawImage(Resources.IMAGE_SLOTMACHINE_OVERLAY, tRect.mX, tRect.mY);
				reanimation.Draw(@new);
			}
			@new.PrepareForReuse();
	...
	public void WhackAZombieSpawning()
	{
		...
		int num7 = 0;
		for (int i = 0; i < aGridPicks.Length; i++)
		{
			aGridPicks[i].Reset();
		}
		int index = -1;
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			...
			{
				Plant topPlantAt = mBoard.GetTopPlantAt(theGridItem.mGridX, theGridItem.mGridY, PlantPriority.TOPPLANT_ONLY_NORMAL_POSITION);
				if (topPlantAt == null || topPlantAt.mSeedType != SeedType.SEED_GRAVEBUSTER)
				{
					aGridPickItemArray[num7] = theGridItem;
					aGridPicks[num7].mItem = num7;
					aGridPicks[num7].mWeight = 1;
					num7++;
				}
			}
		}
		...
		{
			int num8 = (int)TodCommon.TodPickFromWeightedArray(aGridPicks, num7);
			GridItem gridItem = aGridPickItemArray[num8];
			aGridPicks[num8].mWeight = 0;
			if (flag)
			{
				ZombieType[] array = new ZombieType[2]
				{
	...
	public bool UpdateZombieSpawning()
	{
		...
		if (mApp.IsFinalBossLevel() || mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ICE || mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZEN_GARDEN || mApp.mGameMode == GameMode.GAMEMODE_TREE_OF_WISDOM || mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZOMBIQUARIUM || mApp.IsIZombieLevel() || mApp.IsSquirrelLevel() || mApp.IsScaryPotterLevel())
		{
			return true;
		}
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_LAST_STAND && mChallengeState != ChallengeState.STATECHALLENGE_LAST_STAND_ONSLAUGHT)
		{
			return true;
		}
		return false;
	...
	public bool IsStormyNightPitchBlack()
	{
		...
		if (mChallengeState == ChallengeState.STATECHALLENGE_STORM_FLASH_2 && mChallengeStateCounter < 300)
		{
			return false;
		}
		if (mChallengeState == ChallengeState.STATECHALLENGE_STORM_FLASH_3 && mChallengeStateCounter < 150)
		{
			return false;
		}
		return true;
	...
	public void UpdateStormyNight()
	{
		...
		int theMax = ((RandomNumbers.NextNumber(2) != 0) ? 400 : 750);
	...
	public void InitLevel()
	{
		...
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_ZEN_GARDEN)
		{
			...
			mApp.mZenGarden.ZenGardenInitLevel(theJustSwitchingGardens: false);
		}
		if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_COLUMN)
		{
	...
	public void SpawnZombieWave()
	{
		if (mApp.IsContinuousChallenge() && mBoard.mCurrentWave == mBoard.mNumWaves)
		{
			...
			for (int i = 0; i < 50; i++)
			{
				switch (mBoard.mZombiesInWave[mBoard.mCurrentWave, i])
				{
	...
	public void GraveDangerSpawnGraveAt(int x, int y)
	{
		...
		mBoard.AddAGraveStone(x, y)?.AddGraveStoneParticles();
	}
	...
	public void SpawnLevelAward(int theGridX, int theGridY)
	{
		...
		CoinType theCoinType = ((mApp.IsAdventureMode() && mApp.IsFirstTimeAdventureMode()) ? CoinType.COIN_FINAL_SEED_PACKET : ((!mApp.IsAdventureMode() && !mApp.HasBeatenChallenge(mApp.mGameMode)) ? CoinType.COIN_TROPHY : CoinType.COIN_AWARD_MONEY_BAG));
	...
	public void UpdateRainingSeeds()
	{
		...
		if (mChallengeStateCounter == 0)
		{
			...
			while (mBoard.SeedNotRecommendedForLevel(coin.mUsableSeedType) != 0 || !mApp.HasSeedType(coin.mUsableSeedType) || Plant.IsUpgrade(coin.mUsableSeedType) || coin.mUsableSeedType == SeedType.SEED_SUNFLOWER || coin.mUsableSeedType == SeedType.SEED_TWINSUNFLOWER || coin.mUsableSeedType == SeedType.SEED_INSTANT_COFFEE || coin.mUsableSeedType == SeedType.SEED_UMBRELLA || coin.mUsableSeedType == SeedType.SEED_SUNSHROOM || coin.mUsableSeedType == SeedType.SEED_IMITATER);
	...
	public void UpdateConveyorBelt()
	{
		...
		for (int i = 0; i < 20; i++)
		{
			aSeedPickArray[i].Reset();
		}
		int num2 = 0;
		if (mBoard.mLevel == 10)
		{
			aSeedPickArray[num2].mItem = 0;
			aSeedPickArray[num2].mWeight = 20;
			num2++;
			aSeedPickArray[num2].mItem = 2;
			aSeedPickArray[num2].mWeight = 20;
			num2++;
			aSeedPickArray[num2].mItem = 3;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 7;
			aSeedPickArray[num2].mWeight = 20;
			num2++;
			aSeedPickArray[num2].mItem = 5;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 6;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 4;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
		}
		else if (mBoard.mLevel == 20)
		{
			aSeedPickArray[num2].mItem = 11;
			aSeedPickArray[num2].mWeight = 20;
			num2++;
			aSeedPickArray[num2].mItem = 14;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 15;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 12;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 13;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 10;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 8;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
		}
		else if (mBoard.mLevel == 30)
		{
			aSeedPickArray[num2].mItem = 16;
			aSeedPickArray[num2].mWeight = 25;
			num2++;
			aSeedPickArray[num2].mItem = 17;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 18;
			aSeedPickArray[num2].mWeight = 25;
			num2++;
			aSeedPickArray[num2].mItem = 19;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 20;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 21;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 22;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 23;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
		}
		else if (mBoard.mLevel == 40)
		{
			...
			aSeedPickArray[num2].mWeight = 25;
			num2++;
			aSeedPickArray[num2].mItem = 24;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 31;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 27;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 26;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 29;
			aSeedPickArray[num2].mWeight = 25;
			num2++;
			aSeedPickArray[num2].mItem = 28;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 30;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
		}
		else if (mApp.IsFinalBossLevel())
		{
			aSeedPickArray[num2].mItem = 33;
			aSeedPickArray[num2].mWeight = 55;
			num2++;
			aSeedPickArray[num2].mItem = 39;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 20;
			aSeedPickArray[num2].mWeight = 12;
			num2++;
			aSeedPickArray[num2].mItem = 32;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
			aSeedPickArray[num2].mItem = 34;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
			aSeedPickArray[num2].mItem = 14;
			aSeedPickArray[num2].mWeight = 8;
			num2++;
		}
		else if (mApp.IsShovelLevel())
		{
			aSeedPickArray[num2].mItem = 0;
			aSeedPickArray[num2].mWeight = 100;
			num2++;
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_CHALLENGE_WALLNUT_BOWLING_2)
		{
			aSeedPickArray[num2].mItem = 3;
			aSeedPickArray[num2].mWeight = 85;
			num2++;
			aSeedPickArray[num2].mItem = 49;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
			aSeedPickArray[num2].mItem = 50;
			aSeedPickArray[num2].mWeight = 15;
			num2++;
		}
		...
		else if (mApp.IsLittleTroubleLevel())
		{
			...
			num2++;
			aSeedPickArray[num2].mItem = 2;
			aSeedPickArray[num2].mWeight = 35;
			num2++;
		}
		else if (mApp.IsStormyNightLevel())
		{
			aSeedPickArray[num2].mItem = 16;
			aSeedPickArray[num2].mWeight = 30;
			num2++;
			aSeedPickArray[num2].mItem = 26;
			aSeedPickArray[num2].mWeight = 10;
			num2++;
		...
		{
			aSeedPickArray[num2].mItem = 33;
			aSeedPickArray[num2].mWeight = 50;
			num2++;
			aSeedPickArray[num2].mItem = 6;
			aSeedPickArray[num2].mWeight = 25;
			num2++;
		...
		{
			aSeedPickArray[num2].mItem = 33;
			aSeedPickArray[num2].mWeight = 155;
			num2++;
			aSeedPickArray[num2].mItem = 39;
			aSeedPickArray[num2].mWeight = 5;
			num2++;
		...
		{
			TodWeightedArray todWeightedArray = aSeedPickArray[j];
			...
			if (num2 > 2)
			{
				...
				else if (seedType == mLastConveyorSeedType)
				{
					todWeightedArray.mWeight /= 2;
				}
			}
		}
		seedType = (SeedType)TodCommon.TodPickFromWeightedArray(aSeedPickArray, num2);
	...
	public void UpdatePortalCombat()
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE))
			{
				UpdatePortal(theGridItem);
			}
		}
		if (mBoard.HasLevelAwardDropped())
		{
	...
	public GridItem GetOtherPortal(GridItem thePortal)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (thePortal != theGridItem && theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && thePortal.mGridItemType == theGridItem.mGridItemType)
			{
				return theGridItem;
			}
		}
	...
	public void UpdatePortal(GridItem thePortal)
	{
		...
		{
			if (theProjectile.mMotionType != 0 || theProjectile.mRow != thePortal.mGridY || theProjectile.mLastPortalX == thePortal.mGridX)
			{
				continue;
			}
			TRect projectileRect = theProjectile.GetProjectileRect();
			int num5 = projectileRect.mX + projectileRect.mWidth / 2;
			int num6 = thePortal.mGridX * 80 + 55;
			...
			if (num7 <= 40)
			{
				...
				{
					int num8 = num5 - theProjectile.mX;
					int num9 = otherPortal2.mGridY - thePortal.mGridY;
					int num10 = num9 * 100;
					theProjectile.mX = otherPortal2.mGridX * 80 - num8 + 90;
					theProjectile.mPosX = theProjectile.mX;
					theProjectile.mRow = otherPortal2.mGridY;
					theProjectile.mY += num10;
					theProjectile.mPosY = theProjectile.mY;
					theProjectile.mShadowY += num10;
					theProjectile.mLastPortalX = otherPortal2.mGridX;
					theProjectile.mRenderOrder = Board.MakeRenderOrder(RenderLayer.RENDER_LAYER_PROJECTILE, theProjectile.mRow, 0);
				}
			}
		}
		LawnMower theLawnMower = null;
		while (mBoard.IterateLawnMowers(ref theLawnMower))
		{
			if (theLawnMower.mMowerState != LawnMowerState.MOWER_TRIGGERED || theLawnMower.mRow != thePortal.mGridY || theLawnMower.mLastPortalX == thePortal.mGridX)
			{
				continue;
			}
			int num11 = thePortal.mGridX * 80 + 25;
			int num12 = (int)theLawnMower.mPosX + 45;
			int num13 = Math.Abs(num12 - num11);
			if (num13 <= 20)
			{
				GridItem otherPortal3 = GetOtherPortal(thePortal);
				if (otherPortal3 != null)
				{
					int num14 = otherPortal3.mGridY - thePortal.mGridY;
					int num15 = num14 * 100;
					theLawnMower.mPosX = otherPortal3.mGridX * 80 + 25;
					theLawnMower.mRow = otherPortal3.mGridY;
					theLawnMower.mPosY = num15;
					theLawnMower.mLastPortalX = otherPortal3.mGridX;
					theLawnMower.mRenderOrder = Board.MakeRenderOrder(RenderLayer.RENDER_LAYER_LAWN_MOWER, theLawnMower.mRow, 0);
					theLawnMower.Update();
				}
			}
		}
	...
	public float PortalCombatRowSpawnWeight(int theGridY)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE) && theGridItem.mGridY == theGridY)
			{
				flag = true;
				break;
			}
	...
	public GridItem GetPortalToRight(int theGridX, int theGridY)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE) && theGridItem.mGridX - 1 >= theGridX && theGridItem.mGridY == theGridY && (gridItem == null || gridItem.mGridX >= theGridItem.mGridX))
			{
	...
	public GridItem GetPortalAt(int theGridX, int theGridY)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridX == theGridX && theGridItem.mGridY == theGridY && theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE))
			{
				return theGridItem;
			}
		}
	...
	public void MoveAPortal()
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE))
			{
				...
				array[num].mWeight = 1;
				array[num].mItem = theGridItem;
				num++;
			}
		}
		Debug.ASSERT(num != 0);
		GridItem gridItem = (GridItem)TodCommon.TodPickFromWeightedArray(array, num);
		GridItem otherPortal = GetOtherPortal(gridItem);
		...
		for (int j = 2; j < 10; j++)
		{
			for (int k = 0; k < 5; k++)
			{
				if (GetPortalAt(j, k) == null && otherPortal.mGridY != k && gridItem.mGridY != k)
				{
					Debug.ASSERT(num2 < 50);
					array2[num2].mX = j;
					array2[num2].mY = k;
					array2[num2].mWeight = 1;
					num2++;
				}
			}
		}
		TodWeightedGridArray todWeightedGridArray = TodCommon.TodPickFromWeightedGridArray(array2, num2);
		GridItem newGridItem = GridItem.GetNewGridItem();
		newGridItem.mGridItemType = gridItem.mGridItemType;
		newGridItem.mGridX = todWeightedGridArray.mX;
		newGridItem.mGridY = todWeightedGridArray.mY;
		newGridItem.mRenderOrder = Board.MakeRenderOrder(RenderLayer.RENDER_LAYER_PARTICLE, newGridItem.mGridY, 0);
		newGridItem.OpenPortal();
		mBoard.mGridItems.Add(newGridItem);
		gridItem.ClosePortal();
	}
	...
	public GridItem GetPortalToLeft(int theGridX, int theGridY)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_PORTAL_CLOSED && (theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_CIRCLE || theGridItem.mGridItemType == GridItemType.GRIDITEM_PORTAL_SQUARE) && theGridItem.mGridX < theGridX && theGridItem.mGridY == theGridY && (gridItem == null || gridItem.mGridX <= theGridItem.mGridX))
			{
				gridItem = theGridItem;
			}
		}
	...
	public void BeghouledPacketClicked(SeedPacket theSeedPacket)
	{
		...
		{
			if (!BeghouledCanClearCrater() || mChallengeState == ChallengeState.STATECHALLENGE_BEGHOULED_FALLING || mChallengeState == ChallengeState.STATECHALLENGE_BEGHOULED_MOVING)
			{
				return;
			}
	...
	public void ZombiquariumMouseDown(int x, int y)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemType == GridItemType.GRIDITEM_BRAIN)
			{
				num++;
			}
		}
		if (num < 3 && mBoard.TakeSunMoney(5))
		{
	...
	public void ZombiquariumUpdate()
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemType == GridItemType.GRIDITEM_BRAIN)
			{
				theGridItem.mGridItemCounter++;
				theGridItem.mPosY += 0.15f;
				if (theGridItem.mPosY >= 500f)
				{
	...
	public void ScaryPotterOpenPot(GridItem theScaryPot)
	{
		...
		else if (theScaryPot.mGridItemState == GridItemState.GRIDITEM_STATE_SCARY_POT_ZOMBIE)
		{
	...
	public void ScaryPotterJackExplode(int aPosX, int aPosY)
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemType == GridItemType.GRIDITEM_SCARY_POT && theGridItem.mGridX >= num - 1 && theGridItem.mGridX <= num + 1 && theGridItem.mGridY >= num2 - 1 && theGridItem.mGridY <= num2 + 1)
			{
				ScaryPotterOpenPot(theGridItem);
			}
		}
	}
	...
	{
		int num = 0;
		for (int i = 0; i < aPotArray.Length; i++)
		{
			aPotArray[i].Reset();
		}
		int index = -1;
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemState == GridItemState.GRIDITEM_STATE_SCARY_POT_QUESTION && (thePotType != GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF || theGridItem.mScaryPotType == ScaryPotType.SCARYPOT_SEED) && (thePotType != GridItemState.GRIDITEM_STATE_SCARY_POT_ZOMBIE || theGridItem.mZombieType == ZombieType.ZOMBIE_GARGANTUAR))
			{
				aPotArray[num].mItem = theGridItem;
				aPotArray[num].mWeight = 1;
				num++;
				Debug.ASSERT(num <= 54);
			}
		}
		...
		{
			TodWeightedArray todWeightedArray = TodCommon.TodPickArrayItemFromWeightedArray(aPotArray, num);
			todWeightedArray.mWeight = 0;
			GridItem gridItem = (GridItem)todWeightedArray.mItem;
			gridItem.mGridItemState = thePotType;
		}
	}
	...
	public void ScaryPotterPopulate()
	{
		...
		if ((mApp.IsAdventureMode() && mBoard.mLevel == 35) || mApp.mGameMode == GameMode.GAMEMODE_QUICKPLAY_35)
		{
			if (mSurvivalStage == 0)
			{
				ScaryPotterDontPlaceInCol(0, aGridArray, num);
				ScaryPotterDontPlaceInCol(1, aGridArray, num);
				ScaryPotterDontPlaceInCol(2, aGridArray, num);
				ScaryPotterDontPlaceInCol(3, aGridArray, num);
				ScaryPotterDontPlaceInCol(4, aGridArray, num);
				ScaryPotterDontPlaceInCol(5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PEASHOOTER, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 4, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 1, aGridArray, num);
			}
			else if (mSurvivalStage == 1)
			{
				...
				ScaryPotterDontPlaceInCol(3, aGridArray, num);
				ScaryPotterDontPlaceInCol(4, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PEASHOOTER, 4, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 4, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 1, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_FOOTBALL, SeedType.SEED_NONE, 1, aGridArray, num);
			...
			else if (mSurvivalStage == 2)
			{
				...
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PEASHOOTER, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_HYPNOSHROOM, 5, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 6, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 2, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_DANCER, SeedType.SEED_NONE, 1, aGridArray, num);
				ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
		...
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_1)
		{
			...
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_2)
		{
			...
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterDontPlaceInCol(2, aGridArray, num);
			ScaryPotterDontPlaceInCol(8, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 7, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_WALLNUT, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_POTATOMINE, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		...
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_3)
		{
			...
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterDontPlaceInCol(2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_HYPNOSHROOM, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_WALLNUT, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 8, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_DANCER, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		...
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_4)
		{
			ScaryPotterDontPlaceInCol(0, aGridArray, num);
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PUFFSHROOM, 11, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_HYPNOSHROOM, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 8, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 7, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_FOOTBALL, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_5)
		{
			...
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PUMPKINSHELL, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_HYPNOSHROOM, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_MAGNETSHROOM, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_FOOTBALL, SeedType.SEED_NONE, 3, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_6)
		{
			...
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 7, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_TALLNUT, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_THREEPEATER, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_TORCHWOOD, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 7, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_POLEVAULTER, SeedType.SEED_NONE, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_FOOTBALL, SeedType.SEED_NONE, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_7)
		{
			...
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterDontPlaceInCol(2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SPIKEWEED, 13, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_WALLNUT, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 10, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_8)
		{
			ScaryPotterDontPlaceInCol(0, aGridArray, num);
			ScaryPotterDontPlaceInCol(1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PUFFSHROOM, 7, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_TALLNUT, 3, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 8, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 4, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_POGO, SeedType.SEED_NONE, 4, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_9)
		{
			...
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PEASHOOTER, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_THREEPEATER, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_POTATOMINE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_WALLNUT, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PLANTERN, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 8, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_GARGANTUAR, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else if (mApp.mGameMode == GameMode.GAMEMODE_SCARY_POTTER_ENDLESS)
		{
			...
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_LEFTPEATER, 6, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SNOWPEA, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PEASHOOTER, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_THREEPEATER, 2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_SQUASH, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_POTATOMINE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_WALLNUT, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SEED, ZombieType.ZOMBIE_INVALID, SeedType.SEED_PLANTERN, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_SUN, ZombieType.ZOMBIE_INVALID, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_NORMAL, SeedType.SEED_NONE, 8 - num2, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_PAIL, SeedType.SEED_NONE, 5, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_JACK_IN_THE_BOX, SeedType.SEED_NONE, 1, aGridArray, num);
			ScaryPotterPlacePot(ScaryPotType.SCARYPOT_ZOMBIE, ZombieType.ZOMBIE_GARGANTUAR, SeedType.SEED_NONE, 1 + num2, aGridArray, num);
			ScaryPotterChangePotType(GridItemState.GRIDITEM_STATE_SCARY_POT_LEAF, 2);
		}
		else
		{
	...
	public void PuzzleNextStageClear()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				zombie.DieNoLoot(giveAchievements: false);
			}
		}
		...
		{
			if (theCoin.mType == CoinType.COIN_USABLE_SEED_PACKET)
			{
				theCoin.Die();
			}
			else
	...
	public void IZombieMouseDownWithZombie(int x, int y, int theClickCount)
	{
		...
		if (CanPlantAt(num, num2, mBoard.mCursorObject.mType) != 0)
		{
			mBoard.ClearAdvice(AdviceType.ADVICE_NONE);
		...
		ZombieType theZombieType = IZombieSeedTypeToZombieType(mBoard.mCursorObject.mType);
	...
	public void IZombiePlacePlants(SeedType theSeedType, int theCount, int theGridY)
	{
		...
		{
			TodWeightedGridArray todWeightedGridArray = TodCommon.TodPickFromWeightedGridArray(aGridArray, num);
	...
	public void IZombieUpdate()
	{
		...
		{
			if (theParticle.mEffectType == ParticleEffect.PARTICLE_POTATO_MINE)
			{
				flag = true;
				break;
			}
		...
		Coin theCoin2 = null;
		while (mBoard.IterateCoins(ref theCoin2))
		{
			if (theCoin2.IsMoney())
			{
				theCoin2.Die();
	...
	public void IZombieInitLevel()
	{
		...
		else if (mApp.mGameMode == GameMode.GAMEMODE_PUZZLE_I_ZOMBIE_ENDLESS)
		{
			...
			{
				switch (TodCommon.RandRangeInt(0, 4))
				{
				case 0:
			...
			{
				switch (TodCommon.RandRangeInt(0, 5))
				{
				case 0:
				case 1:
				case 2:
	...
	{
		if ((mBoard.mCutScene != null && mBoard.mCutScene.IsBeforePreloading()) || !mApp.Is3DAccelerated())
		{
			return;
		}
		int num = ((mBoard.mX <= 0) ? (mBoard.mX / 100 * -100) : ((mBoard.mX + 100) / 100 * -100));
	...
	public void SquirrelUpdate()
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemType == GridItemType.GRIDITEM_SQUIRREL)
			{
				SquirrelUpdateOne(theGridItem);
			}
		}
	...
	public int SquirrelCountUncaught()
	{
		...
		while (mBoard.IterateGridItems(ref theGridItem, ref index))
		{
			if (theGridItem.mGridItemType == GridItemType.GRIDITEM_SQUIRREL && theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_SQUIRREL_CAUGHT && theGridItem.mGridItemState != GridItemState.GRIDITEM_STATE_SQUIRREL_ZOMBIE)
			{
				num++;
			}
		}
	...
	public void SquirrelFound(GridItem theSquirrel)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			GridItem squirrelAt = mBoard.GetSquirrelAt(num2, num3);
			if (squirrelAt == null)
			{
	...
	public void SquirrelUpdateOne(GridItem theSquirrel)
	{
		...
		if (theSquirrel.mGridItemState == GridItemState.GRIDITEM_STATE_SQUIRREL_WAITING || theSquirrel.mGridItemState == GridItemState.GRIDITEM_STATE_SQUIRREL_ZOMBIE)
		{
			...
			if (theSquirrel.mGridItemCounter == 0)
			{
				if (TodCommon.RandRangeInt(0, 1) == 0 || theSquirrel.mGridItemState == GridItemState.GRIDITEM_STATE_SQUIRREL_ZOMBIE)
				{
	...
	public GridItem IZombieGetBrainTarget(Zombie theZombie)
	{
		...
		TRect tRect = theZombie.GetZombieAttackRect();
		if (theZombie.mZombiePhase == ZombiePhase.PHASE_POLEVAULTER_PRE_VAULT)
		{
			tRect = new TRect(50 + theZombie.mX, 0, 20, 115);
		}
		if (theZombie.mZombieType == ZombieType.ZOMBIE_BALLOON)
		{
			tRect.mX += 25;
		}
		if (tRect.mX > Constants.IZombieBrainPosition)
		{
			return null;
		}
	...
	public void AdvanceCrazyDaveDialog()
	{
		if (mBoard.IsScaryPotterDaveTalking() && mApp.mCrazyDaveMessageIndex != -1)
		{
			...
			else if (mApp.mCrazyDaveMessageIndex == 2702 || mApp.mCrazyDaveMessageIndex == 2801)
			{
				ScaryPotterPopulate();
				mApp.PlayFoley(FoleyType.FOLEY_PLANT);
				mBoard.PlaceRake();
	...
	public bool BeghouledTwistValidMove(int theGridX, int theGridY, BeghouledBoardState theBoardState)
	{
		...
		if (seedType == SeedType.SEED_NONE || seedType2 == SeedType.SEED_NONE || seedType3 == SeedType.SEED_NONE || seedType4 == SeedType.SEED_NONE)
		{
			return false;
		}
		return true;
	...
	public void BeghouledTwistMouseDown(int x, int y)
	{
		...
		int theGridX = 0;
		int theGridY = 0;
		if (!BeghouledTwistSquareFromMouse(x, y, ref theGridX, ref theGridY) || !BeghouledTwistValidMove(theGridX, theGridY, newBeghouledBoardState))
		{
			newBeghouledBoardState.PrepareForReuse();
			return;
		}
		...
		if (!BeghouledTwistMoveCausesMatch(theGridX, theGridY, newBeghouledBoardState))
		{
	...
	public void BeghouledMakeStartBoard()
	{
		...
		BeghouledBoardState newBeghouledBoardState2 = BeghouledBoardState.GetNewBeghouledBoardState();
		LoadBeghouledBoardState(newBeghouledBoardState2);
		BeghouledFillHoles(newBeghouledBoardState2, theAllowMatches: false);
	...
	public void BeghouledCreatePlants(BeghouledBoardState theOldBoardState, BeghouledBoardState theNewBoardState)
	{
		for (int i = 0; i < 8; i++)
		{
			...
			{
				if (theOldBoardState.mSeedType[i, num2] == SeedType.SEED_NONE && theNewBoardState.mSeedType[i, num2] != SeedType.SEED_NONE)
				{
					num++;
					Plant plant = mBoard.NewPlant(i, num2, theNewBoardState.mSeedType[i, num2], SeedType.SEED_NONE);
					plant.mY = 80 - num * 100;
					BeghouledStartFalling(ChallengeState.STATECHALLENGE_BEGHOULED_FALLING);
				}
			}
	...
	public void PuzzlePhaseComplete(int theGridX, int theGridY)
	{
		if (PuzzleIsAwardStage())
		{
			...
			CoinType theCoinType = ((num < 15) ? ((!mApp.mZenGarden.CanDropPottedPlantLoot()) ? CoinType.COIN_AWARD_MONEY_BAG : CoinType.COIN_AWARD_PRESENT) : ((num >= 30) ? CoinType.COIN_AWARD_BAG_DIAMOND : ((!mApp.mZenGarden.CanDropChocolate()) ? CoinType.COIN_AWARD_MONEY_BAG : CoinType.COIN_AWARD_CHOCOLATE)));
	...
	public void TreeOfWisdomDraw(Graphics g)
	{
		...
		mBoard.MouseHitTest(x, y, out var theHitResult, posScaled: false);
		if (theHitResult.mObjectType == GameObjectType.OBJECT_TYPE_TREE_OF_WISDOM && mBoard.mCursorObject.mCursorType == CursorType.CURSOR_TYPE_TREE_FOOD)
		{
			flag = true;
		}
		Reanimation reanimation = mApp.ReanimationGet(mReanimChallenge);
		...
		if (num4 >= 50)
		{
			...
			TodCommon.TodScaleTransformMatrix(ref m, 400f - num6 * 0.5f, 20f + num7 * 0.5f, num5, num5);
			TodCommon.TodDrawStringMatrix(g, Resources.FONT_HOUSEOFTERROR16, m, theString, new SexyColor(255, 255, 255));
		}
	}
	...
	public void TreeOfWisdomNextGarden()
	{
		TreeOfWisdomLeave();
		mApp.KillBoard();
		mApp.PreNewGame(GameMode.GAMEMODE_CHALLENGE_ZEN_GARDEN, theLookForSavedGame: false);
	}
