...
internal class LeaderboardScreen : Widget
{
	...
	public LeaderboardScreen(LawnApp theApp)
	{
		...
		mApp.DelayLoadZenGardenResources(doLoad: false);
		mApp.DelayLoadLeaderboardResource(doLoad: true);
		mZombiePileWidget = new ZombiePileWidget(mApp);
		mPileScrollWidget = new ScrollWidget();
		mPileScrollWidget.EnableBounce(enable: false);
		...
		mPileScrollWidget.ScrollToBottom(animated: false);
	...
	{
		mApp.DelayLoadLeaderboardResource(doLoad: false);
	}
