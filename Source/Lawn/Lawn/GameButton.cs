...
internal class GameButton
{
	...
	public GameButton(int theId, Widget parent)
	{
		...
		for (int i = 0; i < 6; i++)
		{
			ref SexyColor reference = ref mColors[i];
			reference = GameConstants.gGameButtonColors[i];
		}
	...
	public void Draw(Graphics g)
	{
		...
		if (mDrawStoneButton)
		{
			DrawStoneButton(g, mX, mY, mWidth, mHeight, flag, flag2, mLabel, mFont, mFontScale);
			return;
		}
		g.mTransX += mX;
		g.mTransY += mY;
	...
	public void Update()
	{
		...
		if (((uint)mApp.mWidgetManager.mDownButtons & 5u) != 0)
		{
			mIsDown = true;
		}
		else
	...
	{
		DrawStoneButton(g, x, y, theWidth, theHeight, isDown, isHighLighted, theLabel, Resources.FONT_DWARVENTODCRAFT15, 1f);
	}
