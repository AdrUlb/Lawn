...
internal class Dialog : Widget, ButtonListener
{
	...
	public Dialog(Image theComponentImage, Image theButtonComponentImage, int theId, bool isModal, string theDialogHeader, string theDialogLines, string theDialogFooter, int theButtonMode)
	{
		...
		mButtonHeight = theButtonComponentImage?.mHeight ?? ((int)Constants.InvertAndScale(24f));
	...
	{
		return WaitForResult(autoKill: true);
	}

	public virtual int WaitForResult(bool autoKill)
	{
		while (GlobalStaticVars.gSexyAppBase.UpdateApp() && mWidgetManager != null && mResult == int.MaxValue)
		{
		}
		if (autoKill)
		{
