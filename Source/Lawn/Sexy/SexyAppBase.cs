...
internal class SexyAppBase : SexyAppBaseInterface, ButtonListener, DialogListener, IDisposable
{
	...
	public static string mLanguage => Constants.Language.ToString();
	...
	{
		PlaySample(theSoundNum, thePan, looping: false);
	}
	...
	public SexyAppBase(Main m)
	{
		...
		mInitialized = false;
		mMusicEnabled = true;
		mContentManager = ((Game)m).Content;
	...
	{
		return Enumerable.Count<KeyValuePair<int, Dialog>>((IEnumerable<KeyValuePair<int, Dialog>>)mDialogMap);
	}
	...
	public void ShowUpdateRequiredMessage()
	{
		...
		((GameComponent)Main.GamerServicesComp).Enabled = false;
	}
	...
	public void LockOrientation(bool theFlag)
	{
		...
		if (theFlag)
		{
			GraphicsState.mGraphicsDeviceManager.SupportedOrientations = ((Game)XnaGame).Window.CurrentOrientation;
		}
		else
		{
