...
internal class ListWidget : Widget, ScrollListener
{
	...
	public static int[,] gInitialListWidgetColors = new int[6, 3]
	{
		{ 255, 255, 255 },
		{ 255, 255, 255 },
		{ 0, 0, 0 },
	...
	public override void Draw(Graphics g)
	{
		...
		for (int i = num; i <= num2; i++)
		{
			...
			@new.DrawString(theString, mJustify switch
			{
				0 => (int)Constants.InvertAndScale(4f), 
				1 => (mWidth - mFont.StringWidth(theString)) / 2, 
				_ => mWidth - mFont.StringWidth(theString) - (int)Constants.InvertAndScale(4f), 
			}, num4 + num3 - (int)Constants.InvertAndScale(5f));
		}
		if (mDrawOutline)
		{
	...
	public override void MouseMove(int x, int y)
	{
		...
		if (num2 != mHiliteIdx)
		{
			...
			while (listWidget != null)
			{
				listWidget.SetHilite(num2, notifyListener: true);
				listWidget.MarkDirty();
				listWidget = listWidget.mChild;
			}
		}
	...
	public override void MouseLeave()
	{
		...
		while (listWidget != null)
		{
			listWidget.SetHilite(-1, notifyListener: true);
			listWidget.MarkDirty();
			listWidget = listWidget.mChild;
		}
	}
	...
	{
		SetHilite(theHiliteIdx, notifyListener: false);
	}
