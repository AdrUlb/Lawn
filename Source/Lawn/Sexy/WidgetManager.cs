...
internal class WidgetManager : WidgetContainer
{
	...
	protected void SetBaseModal(Widget theWidget, FlagsMod theBelowFlagsMod)
	{
		...
		if (mOverWidget != null && ((uint)mBelowModalFlagsMod.mRemoveFlags & 0x10u) != 0 && IsBelow(mOverWidget, mBaseModalWidget))
		{
			Widget theWidget2 = mOverWidget;
			mOverWidget = null;
			MouseLeave(theWidget2);
		}
		if (mLastDownWidget != null && ((uint)mBelowModalFlagsMod.mRemoveFlags & 0x10u) != 0 && IsBelow(mLastDownWidget, mBaseModalWidget))
		{
		...
		if (mFocusWidget != null && ((uint)mBelowModalFlagsMod.mRemoveFlags & 0x20u) != 0 && IsBelow(mFocusWidget, mBaseModalWidget))
		{
	...
	public void DrawWidgetsTo(Graphics g)
	{
		...
		foreach (Widget mWidget in mWidgets)
		{
			if (mWidget.mVisible)
			{
				...
				@new.SetFastStretch(fastStretch: true);
