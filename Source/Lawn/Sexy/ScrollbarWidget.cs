...
internal class ScrollbarWidget : Widget, ButtonListener
{
	...
	public static int[,] gScrollbarWidgetWidgetColors = new int[6, 3]
	{
		{ 232, 232, 232 },
		{ 48, 48, 48 },
		{ 212, 212, 212 },
	...
	public ScrollbarWidget(int theId, ScrollListener theScrollListener)
	{
		...
		SetColors(gScrollbarWidgetWidgetColors, 6);
	}
	...
	public virtual int GetThumbSize()
	{
		...
		int val = (int)((double)GetTrackSize() * mPageSize / mMaxValue + 0.5);
	...
	public override void Draw(Graphics g)
	{
		...
		int num = 0;
		num = ((!mHorizontal) ? ((mHeight - GetTrackSize()) / 2) : ((mWidth - GetTrackSize()) / 2));
	...
	{
		int num = ((!mHorizontal) ? (y - (mHeight - GetTrackSize()) / 2) : (x - (mWidth - GetTrackSize()) / 2));
