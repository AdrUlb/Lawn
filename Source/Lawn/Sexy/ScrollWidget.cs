...
internal class ScrollWidget : Widget, ProxyWidgetListener
{
	...
	public override void TouchMoved(_Touch touch)
	{
		...
		{
			CGPoint cGPoint2 = GetAbsPos() - mClientLastDown.GetAbsPos();
			CGPoint cGPoint3 = new CGPoint(touch.location.X, touch.location.Y);
			CGPoint cGPoint4 = cGPoint3 + cGPoint2;
			CGPoint cGPoint5 = new CGPoint(cGPoint4.mX + (float)mClientLastDown.mX, cGPoint4.mY + (float)mClientLastDown.mY);
			bool flag = mClientLastDown.GetInsetRect().Contains(cGPoint5);
			...
			CGMaths.CGPointTranslate(ref touch.location, cGPoint2.mX, cGPoint2.mY);
			CGMaths.CGPointTranslate(ref touch.previousLocation, cGPoint2.mX, cGPoint2.mY);
	...
	public override void TouchEnded(_Touch touch)
	{
		...
		{
			CGPoint cGPoint = GetAbsPos() - mClientLastDown.GetAbsPos();
			CGPoint cGPoint2 = new CGPoint(touch.location.X, touch.location.Y);
			_ = cGPoint2 + cGPoint;
			CGMaths.CGPointTranslate(ref touch.location, cGPoint.mX, cGPoint.mY);
			CGMaths.CGPointTranslate(ref touch.previousLocation, cGPoint.mX, cGPoint.mY);
	...
	{
		var color = new Color(255, 255, 255, (int)(255f * mIndicatorsOpacity));
		if (color.A != 0)
		{
	...
	protected Widget GetClientWidgetAt(_Touch touch)
	{
		...
		else
		{
			...
			widgetAtHelper = mClient.GetWidgetAtHelper(num, num2, theFlags, out var _, out theWidgetX, out theWidgetY);
			mClient.mWidgetFlagsMod.mRemoveFlags |= 16;
		}
